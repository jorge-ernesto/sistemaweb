...
class ContribNoHalladoTemplate extends Template {
 
  function titulo(){
    $titulo = '<div align="center"><h2>Contribuyentes no Hallados</h2></div><hr>';
    return $titulo;
  }
  ...
  function listado($registros){
    ...
    $titulo_grid = "CONTRIBUYENTES NO HALLADOS";
    //formulario de busqueda
    $columnas = array('RUC','RAZON SOCIAL','CONDICION');
  ...
  function formBuscar($paginacion){
   //echo "ENTRO BUSCAR\n";
    ...
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MAESTROS.SUNAT_CONTRNOHALL'));
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'SUNAT_CONTRNOHALL'));
  ...
  function formContribNoHallado($datos)
  {
    
    $form = new form2('CONTRIBUYENTES NO HALLADOS', 'form_agen_ret', FORM_METHOD_POST, 'control.php', 'multipart/form-data', 'control');
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MAESTROS.SUNAT_CONTRNOHALL'));
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'SUNAT_CONTRNOHALL'));
