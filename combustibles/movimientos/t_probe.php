...
class ProbeTemplate extends Template {
	...
	function formBuscar($almacen, $dia1, $turno1, $dia2, $turno2, $busqueda, $find) {
		$almacenes = ProbeModel::obtenerAlmacenes("");
		...
		$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden("rqst", "MOVIMIENTOS.PROBE"));
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_combo("almacen", "", $almacen, $almacenes, ""));			
	...
	function listado($resultados, $almacen, $dia1, $turno1, $dia2, $turno2, $busqueda, $find) {
		...
		for ($i = 0; $i < count($res); $i++) {	
			...
			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td align="center" class="'.$color.'">&nbsp;fufufufuf</td>'));
