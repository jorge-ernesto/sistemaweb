	...
	function Init() {
    	...
    	function Run() {
     		...
     		if ($_REQUEST['action']=='Interfaz')
       			$this->visor->addComponent('ContentT', 'content_title', '<div align="center"><h2>Fecha de Aplicacion de Facturas</h2></div><hr>');
     		else	
	    					...
	    					if ($_REQUEST['chkpormonto']=='S') {
			    				...
			    				foreach($_REQUEST['calcular'] as $llave => $valor) {
					   			...
					   			if ($_REQUEST['chkpormontonota']=='S'){
									...
									AplicacionesModel::ActualizarCargos($DatosCargo['datos'][0]['cli_codigo'], $_REQUEST['ch_tipdocumento'],$_REQUEST['ch_numdocumento'],$montosArray[1],$montosArray[2],$montosArray[0]);
									AplicacionesModel::ActualizarCargosNC($DatosCargo['datos'][0]['cli_codigo'], $numdoc[0], $_REQUEST['monto']);
								}
				    			}
	    					}
