...
";
			...
			foreach ($arrData->items as $key => $row) {
	        	// Verificar ITEM
	        	$arrDataSQL = array(
	        		'sql' => "SELECT art_codigo FROM int_articulos WHERE art_codigo = '" . pg_escape_string($row->id_item) . "' AND art_plutipo='1' AND art_estado='0' LIMIT 1;",
	        		'sMessageDanger' => 'Problemas al verificar item',
	        		'sMessageWarning' => 'Movimiento de inventario, no existe item o es un tipo (2 = PLU SALIENTE) o esta desactivado',
	        	);
	        	$arrResponse = $this->checkRegister($arrDataSQL);
	        	if ( $arrResponse['sStatus'] != "success" ) {
		            return $arrResponse;
...
" . pg_escape_string($row->cantidad) . ",
" . pg_escape_string($row->costo_unitario) . ",
" . pg_escape_string($row->costo_promedio) . ",
" . pg_escape_string($row->total) . ",
'" . pg_escape_string($arrData->usuario) . "'
);
        		";
