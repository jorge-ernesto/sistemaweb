...
//10.0.26.1
                ...
                if (importProcess("http://{$rR[0]}/sistemaweb/centralizer.php", $rR[2])) {
                    ...
                    //$Estado = EliminarInfo($datosql);
                   
                    if ($datosql[0]['Infoduplicado'] == 1) {
                        header('Location: eliminarCentrocosto.php?fecha=' . $datosql[0]['f'] . '&c_org_id=' . $datosql[0]['c']);
                    }
...
FROM
	mig_process p
	JOIN mig_remote r USING (mig_remote_id)
WHERE mig_remote_id not in(25,5,7,9,11,13,15,17,20,23,3,27)
GROUP BY
	p.mig_remote_id
ORDER BY
	p.mig_remote_id;";
    ...
    <form id="form1" method="post" action="import_control2.php?cambio=0">
        ...
        <table cellSpacing=3 cellPadding=3>
            ...
            <script type='text/javascript'>
                ...
                function confirmar(url,est,n,obj){  
...
//$y=0;
            ...
            foreach ($crlist as $rV) {
                ...
                ?><input type=button <?php echo $disabled; ?>  id='m<?php echo $j; ?>' name='m<?php echo $j; ?>' value=Centralizar onclick=confirmar('import_control.php?do=migrate&remote=<?php echo $rV[0]; ?>','<?php echo $esta; ?>',<?php echo $n; ?>,this);><?php
        echo "</td>";
