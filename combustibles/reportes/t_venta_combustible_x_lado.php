...
class VentaCombustiblexLadoTemplate extends Template {

	function getTitulo() {
		return '<h2 align="center"><b>Venta Combustible por Lados</b></h2>';
    }
	...
	function formPrincipal($arrAlmacenes, $iAlmacen, $dInicial, $dFinal, $dCierre, $arrLados, $arrLadosForm, $arrProductos, $iProducto, $sTipoVistaDetallado, $sTipoVistaResumido) {
		...
		if ($arrLados['estado']) {
			...
			$counter = 0;
			$checked = array();
			foreach ($arrLados['result'] as $row) {
				$checked[$counter] = "";
				if ( is_array($arrLadosForm) ) {
					for($i = 0; $i < count($arrLadosForm); $i++){
						if($row['nu_lado'] == $arrLadosForm[$i])
							$checked[$counter] = "checked";
					}
					$html_option_lados .= '<input type="checkbox" name="check-arrLados[]" value="' . $row['nu_lado'] . '" ' . $checked[$counter] . '>' . $row['nu_lado'] . ' ';
				} else {
					$html_option_lados .= '<input type="checkbox" name="check-arrLados[]" value="' . $row['nu_lado'] . '" checked>' . $row['nu_lado'] . ' ';
				}
				$counter++;
			}
		} else
			$html_option_lados = $arrLados['mensaje'];
			
		if ($arrProductos['estado']) {
			$html_option = '';
			$html_option = '<option value="0">TODOS</option>';
			foreach ($arrProductos['data'] as $row) {
				$selected = NULL;
				if($row['nu_codigo_producto'] == $iProducto)
					$selected = "selected";
				$html_option .= '<option value="' . $row['nu_codigo_producto'] . '" ' . $selected . '>' . $row['no_producto'] . '</option>';
			}
		} else
			$html_option = $arrProductos['mensaje'];

		$form = new form2('', 'Buscar', FORM_METHOD_POST, 'control.php', '', 'control');
		$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'REPORTES.VENTACOMBUSTIBLEXLADO'));
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<table border="0" align="center" cellpadding="5" cellspacing="5">'));
			...
			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<tr>'));
				$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td align="right">Lado(s)</td>'));
				$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td colspan="3" align="left">'));
			...
			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<tr>'));
				$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td align="right">Producto(s)</td>'));
				$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td colspan="3" align="left">'));
	       			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags(
					'<select id="cbo-iProducto" name="cbo-iProducto">
					    ' . $html_option . '
				    </select>'
					));
	       		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td>'));
