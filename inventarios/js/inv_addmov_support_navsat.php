...
function inicializarVariables($fm,$almacen) {
	...
	$nro_mov = str_pad($nro_mov,7,"0",STR_PAD_LEFT);
...
//	$mov_fecha = "now()"; //inv_movialma.mov_fecha
	...
	/* nro_mov -->Se crea el numero con serie y numero y se hace el insert*/
		...
		$q = "insert into inv_movialma(tran_codigo,mov_almacen
			...
			,com_num_compra,mov_costototal, mov_docurefe2) values
			...
			,'$nro_orden',$mov_costototal,'$nro_lote')";
		pg_exec($q);
		echo "<br>".$q."<br>";
	...
	//preguntar("Enlazar con Cuentas por Pagar?","/sistemaweb/inventarios/js/inv_enlace_cpagar.php",$formulario,$tran_valor);
...
//	$fecha = "now()"; //inv_movialma.mov_fecha --> com_detalle.com_det_fechaentrega 	
	...
	/* nro_mov -->Se crea el numero con serie y numero y se hace el insert*/	
		...
		$nro_orden = str_pad($nro_orden,8,"0",STR_PAD_LEFT);
		}
		...
		?>
			...
			<script language="JavaScript">
				if(confirm('<?php echo $pregunta;?>')){
					window.open('<?php echo $url;?>','miwin','width=650,height=470,scrollbars=yes,menubar=no,left=130	,top=20');		
				}else{
