			...
			FROM 
				(SELECT
				 ...
				 FROM
					...
					int_articulos art	
				 WHERE
					det.pro_codigo = cab.pro_codigo AND
					det.num_tipdocumento = cab.num_tipdocumento AND
					det.num_seriedocumento = cab.num_seriedocumento AND
					det.com_cab_numorden = cab.com_cab_numorden AND
					det.com_cab_numorden like '%$busca%' AND
					art.art_codigo = det.art_codigo AND
					det.pro_codigo='$proveedor' AND
					det.com_det_cantidadpedida <> det.com_det_cantidadatendida AND
					det.com_det_estado <> '5' AND det.com_det_estado <> '3' 
		union
	...
	case "Seleccionar":
		...
		$q2 = "select
			...
			where
				...
				det.num_seriedocumento = cab.num_seriedocumento and
				det.com_cab_numorden = cab.com_cab_numorden and
				det.com_cab_numorden = '$lista' and
				art.art_codigo = det.art_codigo and	
				det.pro_codigo='$proveedor' and
				det.com_det_cantidadpedida <> det.com_det_cantidadatendida";
...
switch($Ingresar){
	case "Ingresar":
			...
			*/
			...
			pg_exec("update com_detalle set com_det_estado='3',current_user_id='$momento' where 
			pro_codigo='$proveedor' and num_tipdocumento='".$S[6]."' and num_seriedocumento='".$S[7]."' 
			and com_cab_numorden='".$S[0]."' and art_codigo='".$S[1]."' ");
...
switch($Eliminar){
	case "Eliminar":
		...
		pg_exec("update com_detalle set com_det_estado='1', current_user_id=null 
		where current_user_id='$momento' and pro_codigo='$proveedor' and num_tipdocumento='".$T[6]."' 
		and num_seriedocumento='".$T[7]."' and com_cab_numorden='".$T[0]."' and art_codigo='".$T[1]."' ");
		}
