<html>
	<head>
		<title>Sistema de Ventas - Reportetest</title>
		<link rel="stylesheet" href="/sistemaweb/css/sistemaweb.css" type="text/css">
		<script language="JavaScript" src="js/kardex.js"></script>
	</head>
	<body>
<?php
include "../menu_princ.php";
//include_once('../include/Classes/PHPExcel.php');
?>

		<div id="header">&nbsp;</div>
		<div id="content">
			<script src="/sistemaweb/js/calendario.js" type="text/javascript" ></script>
			<script src="/sistemaweb/js/overlib_mini.js" type="text/javascript" ></script>
			<div id="content_title">&nbsp;</div>
			<div id="content_body">&nbsp;</div>
			<div id="content_footer">&nbsp;</div>
		</div>
		<div id="footer">&nbsp;</div>
		<iframe id="control" name="control" scrolling="no" src="control.php?rqst=REPORTES.REPORTETURNO" frameborder="1" width="10" height="10"></iframe>
	</body>
</html>
