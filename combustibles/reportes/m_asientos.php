...
class VarillasModel extends Model {
	...
	function obtenerTanques($ch_almacen, $tanque) {
		...
		$sql = "SELECT
			...
			WHERE
				a.dt_fechamedicion>=to_date('" . pg_escape_string($desde) . "', 'dd/mm/yyyy')
			    	and a.dt_fechamedicion <=to_date('" . pg_escape_string($hasta) . "', 'dd/mm/yyyy')
			    	and a.ch_sucursal=trim('" . pg_escape_string($ch_sucursal) . "')
		...
		$flag = VarillasModel::validaDia($dia);
	...
	function obtenerVarilla($sucursal, $dia, $tanque) {
    	...
    	function guardarVarillas($dt_fecha, $ch_sucursal, $ch_tanque, $nu_medicion, $ch_responsable, $usuario, $ip) {
		global $sqlca;
			...
			$sql = "UPDATE
				...
				WHERE
					dt_fechamedicion='$dia'
				    	...
				    	AND ch_tanque='".substr($ch_tanque,0,2)."';";
