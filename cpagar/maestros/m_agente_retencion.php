...
Class AgenteRetencionModel extends Model{

  function eliminarRegistros()
  {
    global $sqlca;
    if($sqlca->perform('sunat_agente_retencion  ', ' ', 'delete', "ch_ruc > '0'")>=0){
  ...
  {
    global $sqlca;
    if($sqlca->perform('sunat_agente_retencion', $datos, 'insert')>=0){
    } else { return $sqlca->get_error();}
    return OK;
  }
  ...
  global $sqlca;
   ...
   if($ext=='txt' || $ext== 'csv' || $ext== 'zip')
   {
	...
	{//INICIO :: IF - Copiar el file al Servidor
	    ...
	    if(!empty($CampoArray))
	    {
                AgenteRetencionModel::eliminarRegistros();
		...
		AgenteRetencionModel::AgregarRegistros($datos[$key]);
	    }
