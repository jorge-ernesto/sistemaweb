...
//	$diad="01"; $mesd=$zmesa; $anod=$zanoa;
	...
	$diad=$diaa-1; $mesd=$zmesa; $anod=$zanoa;
}
...
{
	$fecha_val= $funcion->date_format(trim($fechaa),'DD/MM/YYYY');
}

/*
else{

	$fecha_val= $funcion->date_format(trim($fecha_val),'DD/MM/YYYY');
...
switch ($boton) {
   case Agregar:
			...
			if(strlen(trim($fecha_val))<=0)
			{
				...
				$sql_validar = "select dt_fecha from caj_ta_ajustes_liq where dt_fecha='".$funcion->date_format(trim($fecha_val),'YYYY-MM-DD')."' and ch_sucursal='".$ch_almacen."'";
			//	 echo $sql_validar;

				$xsql_validar = pg_exec($conector_id, $sql_validar);
				
				$a=pg_num_rows($xsql_validar);
				if($a>0)
				{
			...
			if($okgraba==true)
			{
					$sql_insert = "
						...
						,nu_lavados)
						values
						('".$new_estacion."','".$funcion->date_format(trim($fecha_val),'YYYY-MM-DD')."','".$new_aceite."'
						,'".$new_lavado."')";
					//echo $sql_insert;
...
<form name='formular' action="" method="post">
  <table border="1">
	...
	&nbsp;<a href="javascript:show_calendar('formular.diasd');"><img src="/sistemaweb/images/showcalendar.gif" border="0" align="top"/></a><div id="overDiv" style="position:absolute; visibility:hidden; z-index:0;"></th>
      <th>Hasta:</th>
	...
	<tr>
		<th colspan="5" align="left">Sucursal: 
		<?php
