...
SELECT * FROM(
 SELECT
  ...
  (CASE
   ...
   WHEN VC.ch_fac_tipodocumento='35' AND SUBSTRING(VC.ch_fac_seriedocumento FROM '[A-Z]+') IN('B') THEN '00000000008'
  ...
  (CASE
   ...
   WHEN VC.ch_fac_tipodocumento='35' AND SUBSTRING(VC.ch_fac_seriedocumento FROM '[A-Z]+') IN('B') THEN 'Consolidado de boleta de venta electronica'
...
SELECT * FROM(
 ...
 SELECT
  ...
  (CASE
   ...
   WHEN p.td='B' AND FIRST(p.tm) IN('V') AND p.usr!='' THEN '00000000008'
  ...
  (CASE
	...
	WHEN p.td='B' AND FIRST(p.tm) IN('V') AND p.usr!='' THEN 'Consolidado de boleta de venta electronica'
  ...
  SELECT
   ...
   (CASE
	 ...
	 WHEN FIRST(T.td)='B' AND FIRST(T.tm)='V' AND T.usr!='' THEN '00000000008'
	ELSE
	 FIRST(T.ruc)
   END) AS ruc,
   (CASE
	 WHEN FIRST(T.td)='B' AND T.usr='' THEN 'Consolidado ticket boleta'
	 WHEN FIRST(T.td)='B' AND FIRST(T.tm)='V' AND T.usr!='' THEN 'Consolidado de boleta de venta electronica'
	ELSE
	 SUBSTR(FIRST(R.razsocial),0,60)
   END) AS cliente,
