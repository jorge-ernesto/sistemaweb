...
SELECT
 ...
 '' AS no_apellido_materno,
 '' AS no_nombre,
 FIRST(nu_importemovimiento) AS total,
 FIRST(TC.tca_venta_oficial) AS tipocambio,
...
FROM
 ...
 JOIN ccob_ta_detalle AS CTD
  ON(CC.bpartner=CTD.cli_codigo AND CD.doc_type=CTD.ch_tipdocumento AND doc_serial_number=CTD.ch_seriedocumento AND CD.doc_number=CTD.ch_numdocumento AND CD.createdby=CTD.ch_identidad::NUMERIC)
 JOIN int_clientes AS CLI
  ON(CLI.cli_codigo=CC.bpartner)
 JOIN c_cash_transaction_payment AS CP
  USING(c_cash_transaction_id)
