	...
	if ($accion == "tipodocumento") {
		...
		try {
		    ...
		    foreach ($datos_forma_pago as $datos_pagos) {
				...
				// Verificar si el medio de pago es de Retencion
				if ($c_cash_mpayment_id==9) {
					$bRetencion=true;
				}
				// ./ Retencion
		    }
		    ...
		    if ($tipo_accion == "sin_cliente") {
				...
				// Verificar si se activo retencion
				if ( $bRetencion ) {
					// Verificar si el cliente es un agente de Retencion
					$arrParams = array(
						'sNumeroDocumentoIdentidad' => $ruc_cliente,
					);
					$responsePartner = RegistroCajasModel::getPartner($arrParams);
					if ( $responsePartner['sStatus']=='success' ) {
						$sTipoAgente=$responsePartner['sTipoAgente'];
						if ( $sTipoAgente=='R' ) {//R=Retencion
							// Obtener porcentaje de Retencion
							// Parametro -> tax_retention
							$arrParams = array(
								'sTaxRetention' => 'tax_retention'
							);
							$responseTaxRetention = RegistroCajasModel::getTaxRetention($arrParams);
							if ( $responseTaxRetention['sStatus']!='success' ) {
								throw new Exception($responseTaxRetention['sMessage']);									
							}
						}
					} else {
						throw new Exception($responsePartner['sMessage']);
					}
				}
				// ./ Verificar si se activo retencion
				...
				if ($factura_global < $medio_pago) {
					...
					$monto_pago_total_other	= round($importe_global_mp,  2);//IMPORTE DE MEDIO DE PAGO OTROS
					$monto_pago_total	= round($importe_global_mp,  2);//IMPORTE DE MEDIO DE PAGO CON CLIENTE
					$datos_facturas 	= $_REQUEST['datos_factura'];
					$amount				= 0;
					$i = 0;
					$valor = null;
					...
					$monto_pago_total_descontando = (float)$factura_global;

					/*
					Se utilizará cuando la factura o boleta, este enlazada a una N/C o N/D
					pero el usuario no presiona con check en el front-end 
					*/
					$bActivarFlujo = false;
					foreach ($datos_facturas as $rows) {
						$arrRows = explode("*", $rows);
						$iTipoDocumento = trim($arrRows[0]);
						if ( $iTipoDocumento=='20' || $iTipoDocumento=='11') {
							$bActivarFlujo = true;
						}
					}

					$arrParams = array(
						'fTotalRetencion'=>0.00,
					);
					foreach ($datos_facturas as $valores) {
						$informacion 		= explode("*", $valores);
						$tipo_doc 			= $informacion[0];
						...
						if($tipo_doc_emitido == '20'){//NOTA DE CREDITO
							RegistroCajasModel::Insertarc_cuentas_x_cobrar_detalle($ruc_cliente, $tipo_doc_emitido, $tipo_doc_serie, $tipo_doc_numero, '2', '2', $fecha_general, $moneda, $rate, $monto, $reference, $almacen, '', '', '', '', '', '', '', $arrParams);
...
*/
								...
								$fSumTotNC += abs($monto_facturayboleta_restando_nc);
								//$fSumTotND += abs($monto_facturayboleta_aumentando_nd); comentado el 22/04/2019
								$monto_facturayboleta_aumentando_nd=0; // agregado el 22/04/2019

								if ($monto_pago_total_descontando >= $monto) {
									RegistroCajasModel::Insertarc_cuentas_x_cobrar_detalle($ruc_cliente, $tipo_doc_emitido, $tipo_doc_serie, $tipo_doc_numero, '2', '2', $fecha_general, $moneda, $rate, $monto, $reference, $almacen, '', '', $nu_serie, $nu_documento, $i, $datos_facturas, $monto_pago_total_other, $arrParams);
									...
									$monto_pago_total = abs($monto_pago_total) + abs($fSumTotNC) - abs($monto_facturayboleta_aumentando_nd);
									/*
									echo "<pre>";
									echo "FIN (FAC+BOL)+NC-ND -> "; print_r($monto_pago_total); echo "<br>";
									echo "</pre>";
									*/
									RegistroCajasModel::Insertarc_cuentas_x_cobrar_detalle($ruc_cliente, $tipo_doc_emitido, $tipo_doc_serie, $tipo_doc_numero, '2', '2', $fecha_general, $moneda, $rate, $monto_pago_total, $reference, $almacen, '', '', $nu_serie, $nu_documento, $i, $datos_facturas, $monto_pago_total_other, $arrParams);
								}

//						        	echo "con nota de crédito"; echo "<br>";
							} else {
/*
								echo "sin nota de crédito"; echo "<br>";
								...
								echo "<pre>";
*/
								if($tipo_moneda == '01' && $moneda == '02') {
									$monto_pago_total 	= ($monto_pago_total / $rate);
									// guardar array de documento en dolares y pago en soles
									// para luego realizar un UPDATE en ccob_ta_cabecera correctamente
									$arrRegistrosUSD[] = array(
										"iIDTipoDocumento" => $tipo_doc_emitido,
										"sSerieDocumento" => $tipo_doc_serie,
										"sNumeroDocumento" => $tipo_doc_numero,
										"iCodigoCliente" => $ruc_cliente,
										"fImporte" => $monto_pago_total,
									);
								}
								...
								if($monto_pago_total >= $monto){
									...
									RegistroCajasModel::Insertarc_cuentas_x_cobrar_detalle($ruc_cliente, $tipo_doc_emitido, $tipo_doc_serie, $tipo_doc_numero, '2', '2', $fecha_general, $moneda, $rate, $monto_pago_total, $reference, $almacen, '', '', '', '', '', '', '', $arrParams);
						...
						}// /. if NC else Diferente NC
					}// ./ Foerach
				} else if ($factura_global == $medio_pago) {//Pago total
					$tipo_pago = "todo";
					$datos_facturas = $_REQUEST['datos_factura'];
					foreach ($datos_facturas as $valores) {
						$informacion 		= explode("*", $valores);
						...
						// Total de cada comprobante (Boleta / Factura)
						$monto = (float)($monto);
						if ( $bRetencion ) {
							if ( $sTipoAgente=='R' ) {
								// Operar cada factura para obtener el 3% del total.
								$fTotalRetencion=$monto*$responseTaxRetention['fTaxRetention'];
								$arrParams = array(
									'fTotalRetencion'=>$fTotalRetencion,
								);
							}
						}
				...
				} // ./ Pago total de comprobante(s)
			} // ./ Entrar para realizar cobros
			
			if ( isset($arrRegistrosUSD) && is_array($arrRegistrosUSD) ){
				foreach ($arrRegistrosUSD as $row) {
				    RegistroCajasModel::actualizarSaldoSolesaDolares( $row );
				}
			}
