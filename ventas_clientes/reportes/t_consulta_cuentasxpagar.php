...
class ConsultaCuentaxPagarTemplate extends Template {

	function titulo() {
		return '<h2 align="center"><b>Consulta de Saldos de Cuentas por Pagar</b></h2>';
    	}
    
	function FormConsulta($cod_almacen) {

		$sucursales = ConsultaCuentaxPagarModel::obtenerSucursales("");
		$form = new Form('', "Buscar", FORM_METHOD_POST, "control.php", '', "control");	
		$form->addElement(FORM_GROUP_HIDDEN, new form_element_hidden("rqst", "REPORTES.CONSULTAPAGAR"));
	...
	function FormBuscar($resultados,$cobrarcab,$cobrardet,$anticiposcab,$anticiposdet,$valescab,$valesdet,$estacion,$cliente,$razsocial,$documento,$doc,$limite) {
		...
		$result .= '<th class="grid_cabecera"><b>CODIGO PROVEEDOR</b></th>';
		$result .= '<th class="grid_cabecera"><b>RAZON SOCIAL</b></th>';
		$result .= '<th class="grid_cabecera"><b>DOC. POR PAGAR</b></th>';
		$result .= '<th class="grid_cabecera"><b>ANTICIPOS</b></th>';
		$result .= '<th class="grid_cabecera"><b>O/C NO FACTURADAS</b></th>';
		$result .= '</tr>';
		...
		for ($i = 0; $i < count($resultados); $i++) {
			...
			$result .= '<td class="'.$color.'"><A href="control.php?rqst=REPORTES.CONSULTAPAGAR&action=MostrarCab&&estacion='.$_REQUEST['estacion'].'&cliente='.htmlentities($a['cliente']).'&razsocial='.htmlentities($a['razsocial']).'" onClick="foco();" target="control" id="origen"><img src="/sistemaweb/images/all.gif" align="middle" border="0"/></A>&nbsp;</td>';
			$result .= '</tr>';
		...
		if($cobrarcab != "" and $anticiposcab != "" and $valescab != ""){
			...
			$result .= '<tr>';
			$result .= '<th colspan = "7" class="grid_cabecera"><b>DOCUMENTOS DEL PROVEEDOR: ' . htmlentities($razsocial). ' RUC: ' . htmlentities($cliente). '</b></th>';
			$result .= '</tr>';
			$result .= '<tr>';
			...
			$result .= '</tr>';
				...
				$result .= '<td class="'.$color.'"><A href="control.php?rqst=REPORTES.CONSULTAPAGAR&action=Consulta&estacion='.htmlentities($estacion).'" onclick="subir();" target="control"><img src="/sistemaweb/images/arriba.png" align="middle" border="0"/></A>&nbsp;</td>';
				$result .= '<td class="'.$color.'"></td>';
				$result .= '</tr>';
		...
		if($cobrardet != "" and $anticiposdet != "" and $valesdet != ""){
			...
			if(count($cobrardet) == 0 && count($anticiposdet) == 0 && count($valesdet) == 0 ){

				$result .= '<tr bgcolor="">';
				$result .= '<td colspan="5" class="'.$color.'" align = "center">No Hay informacion</td>';
				$result .= '</tr>';
				...
				$result .= '<td ><A href="control.php?rqst=REPORTES.CONSULTAPAGAR&action=MostrarCab&estacion='.htmlentities($estacion).'&cliente='.htmlentities($cliente).'&razsocial='.htmlentities($razsocial).'" onclick="subirdet();" target="control"><img src="/sistemaweb/images/arriba.png" align="middle" border="0"/></A>&nbsp;</td>';
				$result .= '</tr>';
