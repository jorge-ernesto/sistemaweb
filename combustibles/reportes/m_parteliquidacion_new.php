...
class ParteLiquidacionModel extends Model {
	...
	function obtieneParte($desde, $hasta, $estaciones) {
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			if ($a[3]!=''){
				...
				@$result['propiedades'][$propio]['almacenes'][$ch_sucursal]['partes'][$producto]['dia'] = ($a[7] - $a[3] + $a[11]);//Esto es la columna diferencia DIA
			}else{
	...
	function obtieneGLP($desde, $hasta, $estaciones) {
		...
		$sqlA = "select
			...
			from 	(SELECT ch_codigocombustible as combustible,sum(nu_ventavalor) as valor,sum(nu_afericionveces_x_5) * 5 * sum(nu_ventavalor)/sum(nu_ventagalon) as afericion,sum(nu_ventavalor) - (sum(nu_afericionveces_x_5) * 5 * sum(nu_ventavalor)/sum(nu_ventagalon)) as venta 
				FROM comb_ta_contometros 
				WHERE 	ch_codigocombustible='11620307'
					and ch_sucursal='" . pg_escape_string($estaciones) . "' 
					and dt_fechaparte between to_date('" . pg_escape_string($desde) . "', 'DD/MM/YYYY') and to_date('" . pg_escape_string($hasta) . "', 'DD/MM/YYYY')
				GROUP BY ch_codigocombustible) C
...
/*		$sqlB ="SELECT	SUM(CASE WHEN VC.ch_estado='1' THEN VC.nu_importe ELSE 0 END)
	...
	$sqlC = "SELECT
			SUM(CASE WHEN VC.ch_estado='1' THEN VC.nu_importe ELSE 0 END)
		 FROM
			(SELECT ch_estado,ch_documento,ch_cliente,nu_importe,ch_tarjeta,ch_caja,ch_lado,ch_turno 
			FROM val_ta_cabecera 
			WHERE ch_sucursal='".pg_escape_string($estaciones) . "' and dt_fecha between to_date('" . pg_escape_string($desde) . "', 'DD/MM/YYYY') and to_date('" . pg_escape_string($hasta) . "', 'DD/MM/YYYY')) VC 
		INNER JOIN(SELECT ch_documento
			FROM 	val_ta_detalle VTD 
	...
	$almacenes = ParteLiquidacionModel::obtieneListaEstaciones();
		...
		$con1 = "pos_trans".substr($desde,6,4).substr($desde,3,2);
	
		$sqlF = "SELECT sum(importe) FROM $con1 WHERE es = '".pg_escape_string($estaciones)."' AND td = 'F' AND dia BETWEEN to_date('$desde','DD/MM/YYYY HH24:MI:SS') AND to_date('$hasta','DD/MM/YYYY HH24:MI:SS')";

		$sqlB = "SELECT sum(importe) FROM $con1 WHERE es = '".pg_escape_string($estaciones)."' AND td = 'B' AND dia BETWEEN to_date('$desde','DD/MM/YYYY HH24:MI:SS') AND to_date('$hasta','DD/MM/YYYY HH24:MI:SS')";

		$sqlN = "SELECT sum(importe) FROM $con1 WHERE es = '".pg_escape_string($estaciones)."' AND td = 'N' AND dia BETWEEN to_date('$desde','DD/MM/YYYY HH24:MI:SS') AND to_date('$hasta','DD/MM/YYYY HH24:MI:SS')";

		$sqlA = "SELECT sum(importe) FROM $con1 WHERE es = '".pg_escape_string($estaciones)."' AND td = 'A' AND dia BETWEEN to_date('$desde','DD/MM/YYYY HH24:MI:SS') AND to_date('$hasta','DD/MM/YYYY HH24:MI:SS')";

		$sqlD = "SELECT SUM(nu_fac_valortotal) FROM fac_ta_factura_cabecera WHERE ch_almacen = '".pg_escape_string($estaciones)."' AND dt_fac_fecha BETWEEN to_date('$desde','DD/MM/YYYY') AND to_date('$hasta','DD/MM/YYYY')";
