...
Class ContribNoHalladoModel extends Model{

  function eliminarRegistros()
  {
    global $sqlca;
    if($sqlca->perform('sunat_contribuyente_no_hallado  ', ' ', 'delete', "ch_ruc > '0'")>=0){
  ...
  {
    global $sqlca;
    if($sqlca->perform('sunat_contribuyente_no_hallado', $datos, 'insert')>=0){
    } else 
    { 
  ...
  function tmListado($filtro=array(),$pp, $pagina)
  {
    ...
    $cond = '';
    if ($filtro["codigo"] != ""){
      $cond = " WHERE trim(ch_ruc)||''||trim(ch_nombre_razon_social)||''||trim(ch_condicion) ~ '".pg_escape_string($filtro["codigo"])."' ";
    }
...
//ch_ruc,ch_nombre_razon_social,dt_fecha_inicio,nu_resolucion
    $query = "SELECT  ch_ruc, ".
		     ...
		     "ch_condicion ".
             "FROM sunat_contribuyente_no_hallado ".
	     " ".$cond." ".
  ...
  global $sqlca;
   ...
   if($ext=='txt' || $ext== 'csv' || $ext== 'zip')
   {
	...
	{//INICIO :: IF - Copiar el file al Servidor
	    ...
	    foreach($LineArray as $llave => $valor)
	    {
	    ...
	    if(!empty($CampoArray))
	    {
                ContribNoHalladoModel::eliminarRegistros();
