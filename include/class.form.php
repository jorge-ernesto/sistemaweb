...
class form_element_text extends form_element {
	...
	function __construct($title, $name, $value, $separator, $style = "", $size = "", $maxlength=255, $isreadonly=FALSE, $evento="") {
	...
	function getTag() {
		return '<input type="text" name="'.$this->name.'" id="'.$this->name.'" value="'.$this->value.'" class="'.$this->style.'" size="'.$this->size.'" maxlength="'.$this->maxlength.'" '.($this->isreadonly ? ' readonly ' : '').$this->evento.'>'.$this->separator."\n";
	}
}
...
class form_element_combo extends form_element {
	...
	function getTag() {
		$r = '';
		$r .= '<select name="'.$this->name.'" id="'.$this->name.'" class="'.$this->style.'"'.( $this->size != 0 ? ' size="'.$this->size.'"':'').($this->multiple ? ' multiple ' : '').($this->disabled ? ' disabled ' : '').' style="display:'.$this->display.'" '.$this->evento.'>';
...
class form_element_radio extends form_element {
  ...
  var $size;
  var $values;
  function __construct ($title, $name, $value, $separator, $style, $size, $values, $events=[]) {
...
class form_element_checkbox extends form_element {
  ...
  var $size;
  var $values;
  function __construct ($title, $name, $value, $separator, $style, $evento="", $bCheked=false) {
    parent::__construct($title, $name, $value, $separator, $evento);
    if ($style != '') $this->style = $style;
  }
  function getTag () {
    $r = '';
    $r .= '<span class="'.$this->style.'">';
    $r .= '<input type=checkbox name="'.$this->name.'" value="' . $this->value . '"'.($this->value? ' checked':'').' '.$this->evento.'>'.$title.'<br>';
    $r .= '</span>';
    return $r.$this->separator;
  }
}

class form_element_checkbox2 extends form_element {
  var $style = "form_checkbox";
  var $size;
  var $values;
  function form_element_checkbox ($title, $name, $value, $separator, $style, $evento="", $bCheked=false) {
    parent::__construct($title, $name, $value, $separator, $evento);
    if ($style != '') $this->style = $style;
    $this->size = $size;
    $this->values = $values;
  ...
  function getTag () {
    $r = '';
    $r .= '<span class="'.$this->style.'">';
    $r .= '<input type=checkbox name="'.$this->name.'" value="' . $this->value . '"'.($this->value? ' checked':'').' '.$this->evento.'>'.$title.'';
    $r .= '</span>';
    return $r.$this->separator;
  }
}

class form_element_checkbox3 extends form_element {
  var $style = "form_checkbox";
  var $size;
  var $values;
  function form_element_checkbox ($title, $name, $value, $separator, $style, $evento="", $bCheked=false) {
  ...
  function getTag () {
    $r = '';
    $r .= '<span class="'.$this->style.'">';
    $r .= '<input type=checkbox name="'.$this->name.'" value="' . $this->value . '>'.$title.'';
    $r .= '</span>';
    return $r.$this->separator;
  }
}
...
class form_element_freeTags extends form_element {
    ...
    foreach($this->groups as $group_i => $group){
      ...
      for ($element_i=0; $element_i<sizeof ($this->elements[$this->groups[$group_i]->name]); $element_i++)
        ...
        default:
          ...
          $r .= ($element->title!=''?' <span class=form_label >'.$element->title.'</span>'.$this->element_separator : ''). $element->getTag();
          break;
        }
      if ($group->name != FORM_GROUP_MAIN && $group->name != FORM_GROUP_HIDDEN)
        $r .= '</fieldset><br>'."\n";
...
function linea_h($hr=0)
{
  return str_repeat("<hr>",$hr);
}
