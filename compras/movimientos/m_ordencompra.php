...
ORDER BY cast(C.com_cab_numorden as integer) DESC;
	...
	/* ENLAZAR CON MOVIMIENTOS DE INVENTARIOS 01 Y 21 (INGRESOS COMBUSTIBLES Y MARKET) */
	...
	function obtenerTipoMovimientoInventarios() {
		global $sqlca;

		$status = $sqlca->query("SELECT tran_codigo AS id_nu_codigo, tran_codigo || ' - ' || tran_descripcion AS no_descripcion FROM inv_tipotransa WHERE tran_codigo in ('01','21') ORDER BY 2;");
		...
		if($status < 0)
			$arrResult['mensaje'] = 'Error SQL - function obtenerTipoMovimientoInventarios';
		else if($status == 0)
	...
	function obtenerTipoDocumentos() {
		...
		$status = $sqlca->query("SELECT SUBSTR(trim(tab_elemento),5,2) AS id_nu_codigo, tab_descripcion AS no_descripcion FROM int_tabla_general WHERE tab_tabla ='08' AND tab_elemento IN('000009', '000010') ORDER BY 2;");
		...
		if($status < 0)
			$arrResult['mensaje'] = 'Error SQL - function obtenerTipoDocumentos';
		else if($status == 0)
			$arrResult['mensaje'] = 'No se encontró ningún registro';
		else{
			$arrResult['estado'] = TRUE;
	...
	function obtenerTipoMonedas() {
		...
		$status = $sqlca->query("SELECT tab_elemento AS id_nu_codigo, tab_descripcion AS no_descripcion FROM int_tabla_general WHERE tab_tabla ='04' AND tab_elemento !='000000';");
		...
		if($status < 0)
			$arrResult['mensaje'] = 'Error SQL - function obtenerTipoMonedas';
		else if($status == 0)
			$arrResult['mensaje'] = 'No se encontró ningún registro';
		else{
			$arrResult['estado'] = TRUE;
	...
	function saveInventario($data) {
		...
		$iTipoInventario = trim($data['iTipoInventario']);
		$iTipoInventario = strip_tags($iTipoInventario);

		$iTipoDocumento = trim($data['iTipoDocumento']);
		$iTipoDocumento = strip_tags($iTipoDocumento);

		$sSerie = trim($data['sSerie']);
		$sSerie = strip_tags($sSerie);

		$iNumero = trim($data['iNumero']);
		$iNumero = strip_tags($iNumero);

		$Fe_Emision = trim($data['dFechaEmision']);
		$Fe_Emision = strip_tags($Fe_Emision);
		$Fe_Emision = explode("/", $Fe_Emision);
		$Fe_Emision = $Fe_Emision[2] . "-" . $Fe_Emision[1] . "-" . $Fe_Emision[0];

		$arrOrdenCompra = trim($data['arrOrdenCompra']);
		$arrOrdenCompra = strip_tags($arrOrdenCompra);
		$arrOrdenCompra = str_replace(",", "','", $arrOrdenCompra);

		$status = $sqlca->query("
		SELECT
			ORDCAB.com_cab_almacen AS nu_almacen,
			ORDCAB.pro_codigo AS nu_documento_identidad,
			FIRST(ORDCAB.num_tipdocumento) AS nu_tipo_orden_compra,
			FIRST(ORDCAB.num_seriedocumento) AS nu_serie_orden_compra,
			FIRST(ORDCAB.com_cab_numorden) AS nu_numero_orden_compra,
			ORDDET.art_codigo AS nu_id_producto,
			SUM(ORDDET.com_det_cantidadpedida) AS nu_cantidad,
			SUM(ORDDET.com_det_precio) AS nu_costo_unitario,
			SUM(ORDDET.com_det_imparticulo) AS nu_total_sigv
		FROM
			com_cabecera AS ORDCAB
			JOIN com_detalle AS ORDDET USING(pro_codigo, num_tipdocumento, num_seriedocumento, com_cab_numorden)
		WHERE
			ORDCAB.pro_codigo||ORDCAB.num_seriedocumento||ORDCAB.com_cab_numorden IN('" . $arrOrdenCompra . "')
		GROUP BY
			1,2,6
		ORDER BY
			1,
		...
		if($status < 0)
			...
			/* ORDEN DE COMPRA CAMBIANDO A ESTADO -> INVENTARIO */
			$sql = "
			UPDATE
				com_cabecera
			SET
				com_cab_estado = '2'
			WHERE
				pro_codigo||num_seriedocumento||com_cab_numorden IN('" . $arrOrdenCompra . "')
			";

			if($sqlca->query($sql) < 0)
				return $arrResult['estado'] = FALSE;

			$nu_almacen = '';
			foreach ($arrResult['result'] as $row) {

				/* ACTUALIZAR NUMERO MOVIALMA */
				if ($row['nu_almacen'] != $nu_almacen) {
					$nu_almacen = $row['nu_almacen'];

					$sqlca->query("
					UPDATE
						inv_tipotransa
					SET
						tran_nform = (SELECT (tran_nform + 1) FROM inv_tipotransa WHERE tran_codigo = '" . $iTipoInventario . "')
					WHERE
						tran_codigo = '" . $iTipoInventario . "'
					RETURNING
						'" . $nu_almacen . "'||LPAD(CAST((tran_nform::INTEGER) AS bpchar), 7, '0') AS nu_formulario;
					");

					$_row = $sqlca->fetchRow();
					$Nu_Formulario = $_row["nu_formulario"];
				}
				...
				INSERT INTO inv_movialma (
					...
					'" . $Fe_Emision . "',
					'" . $Nu_Formulario . "',
					'" . $row['nu_documento_identidad'] . "',
					'" . $iTipoDocumento . "',
					'" . $sSerie . $iNumero . "',
					'" . $row['nu_tipo_orden_compra'] . "',
					'" . $row['nu_serie_orden_compra'] . "',
					'" . $row['nu_numero_orden_compra'] . "',
					'" . $usuario . "',
					'" . trim($row['nu_id_producto']) . "',
					" . $row['nu_cantidad'] . ",
					" . $row['nu_costo_unitario'] . ",
					" . $row['nu_total_sigv'] . ",
					''
				);
				";

				echo $sql_insert_inv_movialma . "\n";
	...
	function obtenerAlmacenUnico($recordord) {
		...
		$where="";
		if($recordord!="")
			$where = "WHERE ch_sucursal LIKE '" . trim($recordord['com_cab_almacen']) . "'";

		$sql = "
		SELECT
			trim(ch_sucursal) AS cod, ch_nombre_sucursal,
	...
	function obtenerDatosOrden($m_clave,$m_clave2){
		global $sqlca,$sqlca2;
		$query = " 	SELECT   det.com_cab_numorden, 
			    	...
			    	and det.com_cab_numorden='".$m_clave."'
	...
	function obtenerCabecera($numero,$numero2) {
		global $sqlca;
	...
	function obtenerOrdenCorrelativa($alma) {
		global $sqlca;
		$sql =	"SELECT lpad(util_fn_corre_docs('01','".$alma."', 'select' )::text,8,'0')";
		if ($sqlca->query($sql) < 0)
			return false;
		$result = array();
		for ($i=0;$i<$sqlca->numrows();$i++) {
	...
	function obtenerOrdenPorNumero($num_orden) {
		global $sqlca;
		...
		SELECT
			...
			pro.pro_razsocial,
			com.com_factu,
			com.com_cab_observacion,
			com.com_cab_fecharecibida,
			com.com_cab_fechaofrecida,
			com.com_cab_fechaorden,
			com.com_cab_det_glosa
		FROM
			com_cabecera AS com
			JOIN int_proveedores AS pro ON(com.pro_codigo = pro.pro_codigo)
		WHERE
			com.com_cab_numorden = '" . $num_orden . "';
		";
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			$a = $sqlca->fetchRow();
			$result['pro_codigo'] = $a[0];
			$result['percepcion'] = $a[1];
			$result['percepcion_i'] = $a[2];
			$result['razsocial'] = $a[3];
			$result['factura'] = $a[4];
			$result['obs'] = $a[5];
			$result['fecha_entrega'] = $a[6];
			$result['fecha'] = $a[8];
			$result['glosa'] = $a[9];
		}
		return $result;
	...
	function obtenerMonedas() {
		...
		global $sqlca;
		$sql ="select trim(tab_elemento),tab_elemento || ' -- ' || tab_descripcion from int_tabla_general  where tab_tabla='04' and tab_elemento!='000000' order by tab_elemento";
	...
	function buscar($almacen, $dia,$dia2,$pendiente,$inventario,$procesando,$facturado,$cerrado,$percepcion) {
		...
		$sql =	"
		SELECT DISTINCT
			C.com_cab_numorden as OrdenCompra,
			A.ch_nombre_almacen,
			C.pro_codigo as proveedor,
			P.pro_razsocial as nombre,
			to_char(com_cab_fechaorden, 'DD/MM/YYYY') as fecha,
			...
			CASE WHEN cast(com_cab_tipcambio AS integer)!=0 THEN cast(com_cab_tipcambio AS varchar(10)) Else '-' END as T_Cambio,
			com_cab_imporden as importe,
			CASE WHEN dev.mov_numero != '' THEN 'Recibido'  else 'Pendiente' END as estado,
			CASE WHEN com_factu!= '' THEN com_factu Else '-' END  as factura,
			C.num_seriedocumento as SerieCompra,
			TD.tab_desc_breve || ' ' || SUBSTR(MOVI.mov_docurefe, 1, 4) || ' ' || SUBSTR(MOVI.mov_docurefe, 5, 8) AS documento,
			C.com_cab_numorden::INTEGER as OrdenCompra2
		FROM
			com_cabecera AS C
		...
		WHERE
			C.com_cab_fechaorden BETWEEN to_date('".$dia."', 'DD/MM/YYYY') and to_date('".$dia2."', 'DD/MM/YYYY') ";
			if ($almacen != "TODAS") {
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			$resultado[$i]['documento'] = $a[11];
		}
		return $resultado;
	}
//cai
	function buscarPedido($almacen,$dia,$dia2) {
		global $sqlca;
		...
		WHERE 	
			ch_almacen='$almacen' 
			AND dt_fecha between to_date('$dia', 'dd/mm/YYYY') AND to_date('$dia2', 'dd/mm/YYYY')
		ORDER BY 
			id_pedido_cabecera;
		";
	...
	function listar($numPedido) {
		global $sqlca;

		$sql = "
		SELECT 
		art.art_codigo,art.art_descripcion,pcd.ped_cantidad
		from 
		pedido_compra_detalle AS pcd
		LEFT JOIN int_articulos AS art ON (pcd.art_codigo = art.art_codigo)
		WHERE
		id_pedido_cabecera='$numPedido';
		";
	...
	function buscarPedidoDetalle($almacen,$numPedido) {
		global $sqlca;
	    
		$sql = "
		SELECT art_codigo, ped_cantidad
		from pedido_compra_detalle
		WHERE 
			id_pedido_cabecera='77';
		";
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			$a = $sqlca->fetchRow();
			$resultado[$i]['artcodigo']	= $a[0];
			$resultado[$i]['pedcantidad']	= $a[1];
		}
	...
	function agregar($numero,$serie,$fecha, $almacen, $proveedor, $moneda, $tcambio, $m_credito, $fpago, $factura, $comentario, $fentrega, $glosa, $percepcion, $percepcion_i) {
		...
		UPDATE
			int_num_documentos
		SET
			num_numactual = TO_CHAR(" . $numero . ", '99999999')
		WHERE
			num_tipdocumento 		= '01'
...
*/
	...
	function addFlete($Nu_Tipo_Movimiento_Inventario, $Nu_Formulario, $Fe_Emision, $Fe_Flete, $ID_Motivo_Traslado, $No_Placa, $No_Licencia, $No_Certificado_Inscripcion, $ID_Transportista_Proveedor){
		...
		INSERT INTO flete(
			...
			'" . $Fe_Flete . "',
			" . $ID_Motivo_Traslado . ",
			'" . pg_escape_string($No_Placa) . "',
			'" . pg_escape_string($No_Licencia) . "',
			'" . pg_escape_string($No_Certificado_Inscripcion) . "',
			'" . trim($ID_Transportista_Proveedor) . "'
		);
		";
		if ($sqlca->query($sql) < 0)
			return false;
	...
	function updFlete($Nu_Tipo_Movimiento_Inventario, $Nu_Formulario, $Fe_Emision, $Fe_Flete, $ID_Motivo_Traslado, $No_Placa, $No_Licencia, $No_Certificado_Inscripcion, $ID_Transportista_Proveedor){
		global $sqlca;
		...
		SET
			fe_flete = '" . $Fe_Flete . "',
			id_motivo_traslado = '" . $ID_Motivo_Traslado . "',
			no_placa = '" . pg_escape_string($No_Placa) . "',
			no_licencia = '" . pg_escape_string($No_Licencia) . "',
			no_certificado_inscripcion = '" . pg_escape_string($No_Certificado_Inscripcion) . "',
			id_transportista_proveedor = '" . trim($ID_Transportista_Proveedor) . "'
		WHERE
			tran_codigo = '00'
			AND mov_numero = '" . $Nu_Formulario . "'
			AND fe_emision = '" . $Fe_Emision . "';
		";
	...
	function getFlete($Nu_Orden) {
		...
		FROM
			flete AS F
			JOIN int_proveedores AS PROVE ON (F.id_transportista_proveedor = PROVE.pro_codigo)
		WHERE
			tran_codigo = '00'
			AND mov_numero = '" . $Nu_Orden . "'
		";
		...
		if($sqlca->query($sql) < 0)
			$arrResult['mensaje'] = 'Error SQL - function getFlete';
