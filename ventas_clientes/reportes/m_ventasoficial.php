...
class VentasOficialModel extends Model {
	...
	function obtenerEstaciones() {
    	...
    	function reporte($desde, $hasta, $modo, $tipo_cliente, $descontar, $estacion, $seriesdocumentos) {
		global $sqlca;
		...
		if ($desde_mes != $hasta_mes || $desde_ano != $hasta_ano) 
			return false;
		...
		if ($tipo_cliente == "CONTADO" || $tipo_cliente == "AMBAS") {
			...
			/* Nota: este query ha sido tomado de ventas diarias casi sin modificacion. */
		    	$sql = "SELECT
					comb.ch_sucursal as es,
					comb.nu_ventavalor + comb.nu_descuentos as total,
					comb.nu_ventagalon as cantidad,
					round(1+(comb.nu_factor_igv/100), 2) as factor_igv,
			    	        (comb.nu_afericionveces_x_5*5) as afericion,
					(round((comb.nu_ventavalor/comb.nu_ventagalon), 6)) as precio,
					comb.ch_codigocombustible as codigo,
					to_char(comb.dt_fechaparte, 'dd/mm') as dia,
					(SELECT sum(afe.importe) FROM pos_ta_afericiones afe WHERE afe.dia = comb.dt_fechaparte AND afe.pump=lpad(surt.ch_numerolado,2,'0') AND afe.codigo = surt.ch_codigocombustible AND afe.es = comb.ch_sucursal) AS impafe
				FROM
		    			comb_ta_contometros comb
					JOIN comb_ta_surtidores surt ON (comb.ch_sucursal = surt.ch_sucursal AND comb.ch_surtidor = surt.ch_surtidor)
				WHERE
				    	comb.dt_fechaparte BETWEEN '".pg_escape_string($desde_ano.'-'.$desde_mes.'-'.$desde_dia)."' AND '".pg_escape_string($hasta_ano.'-'.$hasta_mes.'-'.$hasta_dia) . "'
					AND comb.nu_ventavalor > 0
		    	        	AND comb.nu_ventagalon > 0 ";

		    	if ($estacion != "TODAS") {
	    			$sql .= "AND comb.ch_sucursal='".pg_escape_string($estacion)."'";
		    	}
				...
				/* Descuenta afericiones y actualiza totales en valor con la nueva venta */
		    	    	$cantidad -= $afericion;
				$total 	  -= ($impafe);   //($afericion*$precio);
				$neto 	   = round($total/$factor_igv, 2);
				$igv 	   = round($total-$neto, 2);
		
				/* El modelo de este reporte exige que existan todos los dias en el rango especificado, aunque sean llenos de ceros. 
				   Aqui verificamos si existe el dia indicado y no existiera, lo creamos lleno de ceros. */
			...
			$sql = "SELECT (
				    	SELECT
						CASE WHEN min(m.trans)='1' AND max(m.trans)='9999' 
						THEN (SELECT m3.trans FROM pos_trans".pg_escape_string($desde_ano.$desde_mes)." m3 
						WHERE m3.dia=pt.dia AND m3.caja=pt.caja AND m3.es=pt.es 
						ORDER BY m3.fecha ASC LIMIT 1) ELSE min(m.trans) END
				    	FROM 
					...
					(
				    	SELECT 
						CASE WHEN min(m2.trans)='1' AND max(m2.trans)='9999' 
						THEN (SELECT m3.trans FROM pos_trans".pg_escape_string($desde_ano.$desde_mes)." m3 
						WHERE m3.dia=pt.dia AND m3.caja=pt.caja AND m3.es=pt.es 
						ORDER BY m3.fecha DESC LIMIT 1) ELSE max(m2.trans) END
				    	FROM 
			...
			// echo "<script>console.log('" . json_encode($ventas) . "')</script>";
		}
		/*-----------------> Fin de parte 1 <------------------------*/
		...
		if ($tipo_cliente == "CONTADO" || $tipo_cliente == "AMBAS") {
			...
			// echo "</pre>\n";
				...
				switch ($tipo) {
					case '02':
						$tipo = "lubricantes";  break;
				    	case '03':
						$tipo = "accesorios";	break;
				    	case '06':
						$tipo = "servicios";	break;
				    	case '05':
						$tipo = "market";	break;
				    	case '08':
						$tipo = "market";	break;
		...
		if (($descontar == 'S' && $tipo_cliente != 'CREDITO') || $tipo_cliente == "CONTADO") {
			...
			$sql = "SELECT
					...
					round(1+(nu_factor_igv/100), 2) as factor_igv
				FROM
					val_ta_detalle det,
					int_articulos art
				WHERE 
					det.dt_fecha BETWEEN '" . $desde_ano."-".$desde_mes."-".$desde_dia."' AND '" . $hasta_ano."-".$hasta_mes."-".$hasta_dia."' ";
		 ...
		 */
	    	...
	    	$sql = "SELECT
				...
				LEFT JOIN fac_ta_factura_complemento com ON (cab.cli_codigo = com.cli_codigo AND cab.ch_fac_seriedocumento=com.ch_fac_seriedocumento AND cab.ch_fac_numerodocumento=com.ch_fac_numerodocumento AND cab.ch_fac_tipodocumento=com.ch_fac_tipodocumento),
		        	int_clientes cli,
		...
		/* limitando las series y tipos de documento a mostrar */
		if (count($seriesdocumentos) > 0) { 
			$sql .= " AND cab.ch_fac_tipodocumento in (";
			for ($i = 0; $i < count($seriesdocumentos); $i++) {
				$tipdocu = substr($seriesdocumentos[$i],0,2);
				if ($i > 0)
					$sql .= ",";
				$sql .= "'" . pg_escape_string($tipdocu) . "'";
			}
			$sql .= ") ";

			$sql .= " AND cab.ch_fac_seriedocumento in (";
			for ($i = 0; $i < count($seriesdocumentos); $i++) {
				$serdocu = substr($seriesdocumentos[$i],3,3);
				if ($i > 0)
					$sql .= ",";
				$sql .= "'" . pg_escape_string($serdocu) . "'";
			}
			$sql .= ") ";
		} else {
			$sql .= " AND cab.ch_fac_tipodocumento IN ('10', '11', '20', '35')";
		}
		...
		$cuenta = 0;
			...
			if ($anulado != 'S') {
			    	...
			    	$ventas['documentos']['tipos'][$td]['paginas'][$pagina]['documentos'][$documento]['cliente'] 	= $cliente;
		...
		/* limitando las series y tipos de documento a contabilizar */
		if (count($seriesdocumentos) > 0) { 
			$sql .= " AND cab.ch_fac_tipodocumento in (";
			for ($i = 0; $i < count($seriesdocumentos); $i++) {
				$tipdocu = substr($seriesdocumentos[$i],0,2);
...
//	ksort($ventas['combustibles']['estaciones']);
	...
	/************************ Fin de parte 5 **************************/
		...
		// cond1: en comentario para cualquier cliente menos ultracom
		/*$sql = $sql . "
			GROUP BY
			    	t.fecha,
			    	t.td,
		...
		// cond2: activo solo para ultracom
		$sql = $sql . "
			GROUP BY
			    	t.fecha,
			    	t.td,
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			$td 		= $a['td'];
		    	...
		    	$ventas['tickets_factura']['total_cantidad'] 	+= $cantidad;
	    		...
	    		$ventas['tickets_factura']['estaciones'][$es]['total_cantidad']	+= $cantidad;	    		
	    		...
	    		$ventas['tickets_factura']['estaciones'][$es]['detalle'][$i]['cantidad'] 	= $cantidad;	    		
		}
		/************************ Fin de parte 6 **************************/
	...
	function obtenerSeries() {
		...
		$sql = "SELECT	
			...
			FROM
				int_num_documentos
			WHERE
				num_tipdocumento IN ('10','11','03','20','21','35','22') 
			ORDER BY 
				num_tipdocumento,
				num_seriedocumento,
				num_descdocumento;";
	...
	function obtenerAlmaActual($almacen) {
		global $sqlca; 
