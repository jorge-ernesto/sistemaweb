...
ORDER BY
 ch_sucursal
  ...
  function getManualInvoiceSale( $arrParams, $postrans ){
    global $sqlca;
...
SELECT
 ...
 ( SELECT fpago FROM " . $postrans . " WHERE usr ~* CAST( (string_to_array(VCOM.ch_fac_observacion2, '*'))[2] || '-' || (string_to_array(VCOM.ch_fac_observacion2, '*'))[1] AS text) LIMIT 1 ) as fpago_referencia,
 ( SELECT at FROM " . $postrans . " WHERE usr ~* CAST( (string_to_array(VCOM.ch_fac_observacion2, '*'))[2] || '-' || (string_to_array(VCOM.ch_fac_observacion2, '*'))[1] AS text) LIMIT 1 ) as at_referencia
FROM
...
ORDER BY
 ...
 numero
    ...
    function getManualInvoiceSaleReceivable( $arrParams, $postrans ){
      global $sqlca;
...
SELECT * FROM(
 SELECT
  ...
  ''::TEXT AS turno,
	''::TEXT AS txt_glosa, --1
  ( SELECT fpago FROM " . $postrans . " WHERE usr ~* CAST( VC.ch_fac_seriedocumento||'-'||VC.ch_fac_numerodocumento::TEXT AS text ) LIMIT 1 ) as fpago,
  ( SELECT at FROM " . $postrans . " WHERE usr ~* CAST( VC.ch_fac_seriedocumento||'-'||VC.ch_fac_numerodocumento::TEXT AS text ) LIMIT 1 ) as at
 FROM
  fac_ta_factura_cabecera AS VC
  LEFT JOIN fac_ta_factura_complemento AS VCOM
   USING (ch_fac_tipodocumento, ch_fac_seriedocumento, ch_fac_numerodocumento, cli_codigo)
...
SELECT DISTINCT
 ...
 '2'::TEXT AS orden,
 ''::TEXT AS turno,
 CM.name||' - '||CP.pay_number::TEXT AS txt_glosa, --3
 ( SELECT fpago FROM " . $postrans . " WHERE usr ~* CAST( ''||'-'||CP.pay_number AS text ) LIMIT 1 ) as fpago,
 ( SELECT at FROM " . $postrans . " WHERE usr ~* CAST( ''||'-'||CP.pay_number AS text ) LIMIT 1 ) as at
FROM
...
SELECT DISTINCT
 ...
 '2'::TEXT AS orden,
 ''::TEXT AS turno,
 CM.name||' - '||CP.pay_number::TEXT AS txt_glosa, --4
 ( SELECT fpago FROM " . $postrans . " WHERE usr ~* CAST( ''||'-'||CP.pay_number AS text ) LIMIT 1 ) as fpago,
 ( SELECT at FROM " . $postrans . " WHERE usr ~* CAST( ''||'-'||CP.pay_number AS text ) LIMIT 1 ) as at
FROM
 c_cash_transaction AS CC
...
SELECT
 ...
 (CASE
 WHEN TRIM(FIRST(CLI.cli_ruc))='99999999999' THEN '0'
 WHEN TRIM(FIRST(CLI.cli_ruc))='999999999' THEN '0'
 WHEN TRIM(FIRST(CLI.cli_ruc))='11111111' THEN '0'
 WHEN CHAR_LENGTH(TRIM(FIRST(CLI.cli_ruc)))=11 THEN '6'
 WHEN CHAR_LENGTH(TRIM(FIRST(CLI.cli_ruc)))=8 THEN '1'
 ELSE
 '0'
 END) AS tipodi,
 (CASE WHEN FIRST(CD.doc_type='10') THEN FIRST(CLI.cli_ruc) ELSE FIRST(cuentas.id_cliente_boleta) END) AS ruc,
 (CASE WHEN FIRST(CD.doc_type='10') THEN FIRST(CLI.cli_razsocial) ELSE FIRST(cuentas.no_rsocial_cliente_boleta) END) AS cliente,
 '' AS no_apellido_paterno,
 '' AS no_apellido_materno,
 '' AS no_nombre,
 FIRST(nu_importemovimiento) - COALESCE(FIRST(rate_gn), 0) AS total,
 FIRST(TC.tca_venta_oficial) AS tipocambio,
 ...
 ''::TEXT AS turno,
 ''::TEXT AS txt_glosa, --5
 ( SELECT fpago FROM " . $postrans . " WHERE usr ~* CAST( CD.doc_serial_number||'-'||CD.doc_number::TEXT AS text ) LIMIT 1 ) as fpago,
 ( SELECT at FROM " . $postrans . " WHERE usr ~* CAST( CD.doc_serial_number||'-'||CD.doc_number::TEXT AS text ) LIMIT 1 ) as at
FROM
 c_cash_transaction AS CC
 JOIN c_cash_transaction_detail AS CD
  USING(c_cash_transaction_id)
 JOIN ccob_ta_detalle AS CTD
  ON(CC.ware_house=CTD.ch_sucursal AND CC.bpartner=CTD.cli_codigo AND CD.doc_type=CTD.ch_tipdocumento AND doc_serial_number=CTD.ch_seriedocumento AND CD.doc_number=CTD.ch_numdocumento AND CD.createdby=CTD.ch_identidad::NUMERIC)
 JOIN int_clientes AS CLI
  ON(CLI.cli_codigo=CC.bpartner)
 JOIN c_cash_transaction_payment AS CP
  USING(c_cash_transaction_id)
...
WHERE
 ...
 AND EXTRACT(YEAR FROM CC.d_system)='".$arrParams["dYear"]."'
 AND EXTRACT(MONTH FROM CC.d_system)='".$arrParams["dMonth"]."'
 AND CP.c_cash_mpayment_id != 9--No se muestra el genera asiento contable para retencion
GROUP BY
 1,2,3,4,CC.transaction
)
ORDER BY
 nucorrelativo,
 orden,
...
SELECT * FROM(
 SELECT
  ...
  ''::TEXT AS at,
  CASE
	WHEN FIRST(p.td) = 'B' OR FIRST(p.td) = 'F' THEN COALESCE( SUM(p.balance), 0.00 )
	ELSE 0.00
...
ORDER BY
 ...
 trans;
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			$result[$key_array][$correlativo]['fpago'] = $a['fpago']; //Agregado 2020-01-20
			$result[$key_array][$correlativo]['at']    = $a['at'];    //Agregado 2020-01-20
			$result[$key_array][$correlativo]['balance'] = $a['balance']; //Agregado 2021-02-04 //ICBPER
		...
		for ($i = 0; $i < count($result['ticket_tmp']); $i++) {
	  	...
	  	$result['ticket'][$correlativo]['numero'] = $inicio_boleta."-".$fin_boleta;
	  	$result['ticket'][$correlativo]['serie'] = $serie_agrupar;
		$result['ticket'][$correlativo]['balance'] = $array_tmp_impresion['balance']; //ICBPER
	  	$result['ticket'][$correlativo]['importe'] = $array_tmp_impresion['importe'];
		return $result;
	}
	...
	function llenar_arreglo_objecto_imprimir_Cobranza($array_tmp_impresion, $a) {
		...
		$array_tmp_impresion['fpago'] = $a['fpago']; //Agregado 2020-01-20
		$array_tmp_impresion['at']    = $a['at'];    //Agregado 2020-01-20
		$array_tmp_impresion['balance'] += $a['balance'];    //Agregado 2021-02-04 //ICBPER
	...
	function AsientosVentas($almacen, $postrans, $fecha_serie, $tipo, $decimales, $iTipoVenta, $iConNotaDespacho) {
       ...
       END
			...
			ext.fpago_referencia as fpago_referencia,
			ext.at_referencia as at_referencia,
			CASE
				WHEN FIRST(T.td) = 'B' OR FIRST(T.td) = 'F' THEN COALESCE( SUM(T.balance), 0.00 )
				ELSE 0.00
			END AS balance --ICBPER
		FROM
			...
			(SELECT
				...
				venta_tickes.fpago_referencia as fpago_referencia,
        		venta_tickes.at_referencia as at_referencia
			FROM
				(SELECT 
					...
					p.fecha AS fefecha,
					p.fpago as fpago_referencia,
            		p.at as at_referencia,
					(CASE
				...
				GROUP BY
					...
					venta_tickes.nu_tipo_referencia,
					venta_tickes.fpago_referencia,
            		venta_tickes.at_referencia
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			$result[$key_array][$correlativo]['fpago']	                = $a['fpago']; //Agregado 2020-01-16
			$result[$key_array][$correlativo]['at']	                    = $a['at'];    //Agregado 2020-01-16
			$result[$key_array][$correlativo]['fpago_referencia']	    = $a['fpago_referencia']; //Agregado 2020-01-16
			$result[$key_array][$correlativo]['at_referencia']	        = $a['at_referencia'];    //Agregado 2020-01-16
			$result[$key_array][$correlativo]['balance']	            = $a['balance'];    //Agregado 2020-02-04 //ICBPER
...
*/
	...
	function llenar_arreglo_objecto_imprimir($array_tmp_impresion, $a) {
		...
		$array_tmp_impresion['fpago']              = $a['fpago'];
		$array_tmp_impresion['at']                 = $a['at'];
		$array_tmp_impresion['fpago_referencia']   = $a['fpago_referencia'];
		$array_tmp_impresion['at_referencia']      = $a['at_referencia'];
...
*/
		...
		if ( abs($fIGV) > 0.00 ){
			...
			$array_tmp_impresion['balance']	    += $a['balance']; //ICBPER
			$array_tmp_impresion['importe']	    += $a['importe'];
		} else {
		...
		try {
          ...
          JOIN inv_ta_almacenes AS ALMA ON(FC.ch_almacen = ALMA.ch_almacen)
				...
				(SELECT
					...
					CASE
						WHEN FIRST(PT.td) = 'B' OR FIRST(PT.td) = 'F' THEN COALESCE( SUM(PT.balance), 0.00 )
						ELSE 0.00
					END AS balance --ICBPER
				FROM
					" . $postrans . " AS PT
