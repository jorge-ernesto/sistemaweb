...
class f2element_text extends form2_element {
  ...
  function getTag () {
    return '<input type="text" name="'.$this->nameid.'" value="'.$this->value.'" '.getAttributes($this->attributes).' '.getParameters($this->parameters).'>'.$this->separator."\n";
  }
}
...
class f2element_textarea extends form2_element {
  ...
  function __construct ($nameid, $lavel, $value, $separator, $cols, $rows, $attributes=array(), $parameters=array()) {
    parent::__construct($nameid, $label, $value, $separator, $attributes, $parameters);
    if (!isset($this->attributes["class"])) $this->attributes["class"] = $this->class;
...
class f2element_combo extends form2_element {
  ...
  function getTag () {
    $r = '';
    $r .= '<select name="'.$this->nameid.'" '.getAttributes($this->attributes).' '.getParameters($this->parameters).'>';
...
var $class = "form_file";
  ...
  function __construct ($name, $value, $separador, $attributes=array(), $parameters=array()) 
  {
    parent::__construct($nameid, '', $value, $separador, $attributes, $parameters);
    if (isset($this->attributes["class"])) $this->attributes["class"] = $this->class;
    $this->attributes["size"] = $size;
...
class form2 {
  ...
  function __construct ($title, $name, $method, $action, $enctype='', $target='') {
    $this->title = $title;
    $this->name = $name;
    $this->method = $method;
    $this->action = $action;
