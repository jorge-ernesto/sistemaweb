...
function Build(p_item, p_month, p_year, p_format) {
	...
	if (ggPosX == -1 && ggPosY == -1) {
		overlib(ggWinContent, AUTOSTATUSCAP, STICKY, CLOSECLICK, CSSSTYLE,
			...
			CAPTION, "Seleccione una fecha", OFFSETX, 20, OFFSETY, -20);
...
function show_calendar() {
	...
	*/
	...
	realFieldName = p_item;
	separatorIndex = p_item.indexOf(".");
	if (separatorIndex > 0)
		realFieldName = realFieldName.substring((separatorIndex + 1));
	eList = document.getElementsByName(realFieldName);
	if (eList.length < 1)
		realField = null;
	else
		realField = eList[0];
	...
	if (realField != null) {
		if (p_format == "DD\/MM\/YYYY") {
			decDate = realField.value.split("\/");
			if (decDate.length == 3) {
				p_month = new String(parseInt(decDate[1]) - 1);
				p_year = decDate[2];
			}
...
/*	if (arguments[4]==null)
	...
	if (realField != null && ggPosX) {
		realPosition = cumulativeOffset(realField);
		ggPosX = realPosition.left;
		ggPosY = realPosition.top + realField.offsetHeight;
	}
