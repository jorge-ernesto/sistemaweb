...
class RegistroCajasTemplate extends Template {
    ...
    function titulo() {
        return '<h2 align="center"><b>Recibo de Egreso de Caja</b></h2>';
    }
    ...
    function FormularioPrincipal() {
        ...
        <div style="text-align: center;position: relative;" id="id_nuevo_registro_view">
            ...
            <div ><h3 style="color: #336699; text-align: center;">Recibo de Egreso de Caja.</h3></div>
			...
			<input onclick="ViewClient(this)" type="radio" name=myclient value="N" >No
                ...
                <td> <input type='text' id='id_cliente_auto' class='fecha_formato' style="width: 250px;" onkeyup="autocompleteBridge(0)"></td>
                </tr>
                <tr class="ayuda_clientes_id" style="display: none;">
                    <td>Ruc proveedor  </td>
                ...
                <tr>
                    ...
                    <td colspan="2"><button id="id_nuevo_registro"><img align="right" src="/sistemaweb/icons/agregar.gif">Nuevo Recibo</button> 
                   <button id="executar_excel"><img align="right" src="/sistemaweb/icons/gexcel.png">Excel</button></td>
                    <td align="left"><button id="id_nuevo_cuenta_bank" style="display: none;"><img align="right" src="/sistemaweb/icons/agregar.gif">Cuenta Bancaria</button> </td>
                </tr>
            </table>
	...
	function FormularioPrincipalSegundario($estaciones, $caja, $operacion, $almacen, $fecha_actual, $serie, $fecha, $monedas) {

        ?>

        <div ><h3 style="color: #336699; text-align: center;">Nuevo Recibo de Egreso</h3></div>
        <div id="cargardor" style="position: absolute;display: inline;z-index: 999;"><img src="movimientos/cg.gif" /></div>
        <table style="text-align: left;position: relative;margin: 5px auto;">
            <tr>
                <td>Sucursal: </td>
            ...
            <tr>
                <td>Tipo Cambio  </td>
                <td> <input type='text' id='txttipo_cambio' class='fecha_formato' value="<?php echo $fecha;?>"/> </td>
            </tr>
    ...
    function CrearTablareporte($registros,$sucursal) {
        ?>
        <!--<div style="float: left;width: auto;border: 1px;color:red;">-->
        <table cellspacing="0" cellpadding="0" border="0" style="text-align: left;position: relative;margin: 20px auto;">	
            <thead>
        ...
        foreach ($registros as $llave => $value) {
            ...
            echo "<tr class='$estila'>";
            	...
            	echo '<td><button onclick="verdetalle(\'' . $value['id_recibo'] . '\', \'' . $sucursal . '\')">Ver Recibo</button></td>';
              	echo '<td><button onclick="anular(\'' . $value['id_recibo'] . '\', \'' . $value['ware_house'] . '\', \'' . $value['d_system'] . '\')">Anular</button></td>';
        	echo "</tr>";
	...
	function CrearTablaSeleccionarClienteDetalleRecibo($registros, $dat_medio_pago, $dat_banco, $tc, $moneda) {
        ...
        ?>
			...
			foreach ($registros as $llave => $value) {
				...
				if($moneda == '01' && $value['ch_moneda'] == '02')//SI MI CAJA ES EN SOLES Y DOCUMENTO PAGO EN DOLARES
					$amount = ($value['nu_importesaldo'] * $tc);
				elseif($moneda == '02' && $value['ch_moneda'] == '01')//SI MI CAJA ES EN DOLARES Y DOCUMENTO PAGO EN SOLES
					$amount = ($value['nu_importesaldo'] / $tc);
				elseif($moneda == '02' && $value['ch_moneda'] == '02')//SI MI CAJA ES EN DOLARES Y DOCUMENTO PAGO EN DOLARES
					$amount = $value['nu_importesaldo'];
				else
					$amount = $value['nu_importesaldo'];
				...
				echo "<td align='center'>" . $value['dt_fechaemision'] . "</td>";
				echo "<td align='center'>" . $value['dt_fechavencimiento'] . "</td>";
				//echo "<td>" . $value['ch_numdocreferencia'] . "</td>";
				echo "<td align='center'>" . $value['moneda'] . "</td>";
				echo "<td align='right' class='td_tabla_selecinar'>" . number_format($value['nu_importetotal'], 2) . "</td>";
				echo "<td align='right' class='td_tabla_selecinar'>" . number_format($value['nu_importesaldo'], 2) . "</td>";
				...
				echo "<input type='hidden' class='fac_insertar' name='doc_$i' value='" . $value['tipo_doc'] . "*" . $datos_partidos . "*" . $value['nu_importesaldo'] . "*" . $value['ch_numdocreferencia'] . "*" . $value['ch_moneda'] . "*" . $value['tipo_emitido'] . "' /></td>";
//				echo "<td class='td_tabla_selecinar'>" . number_format($value['nu_importesaldo'], 2) . "<input type='hidden' class='fac_insertar' name='doc_$i' value='" . $value['tipo_doc'] . "*" . $datos_partidos . "*" . $value['nu_importesaldo'] . "*" . $value['ch_numdocreferencia'] . "*" . $value['ch_moneda'] . "*" . $value['tipo_emitido'] . "' /></td>";
