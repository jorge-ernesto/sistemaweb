...
class VentasOficialTemplate extends Template {
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_text('desde', '', date('d/m/Y'), '', 12, 10,array(),''));
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_text('hasta', '', date('d/m/Y'), '', 12, 10,array(),''));
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_text('impresion', '', date("d/m/Y"), '', 12, 10,array(),''));
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>&nbsp</td>'));
		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td><input type="radio" name="condicion" value="seriedocumento" onclick="cargarListaSerie();">Serie de Documento</td></tr>'));
		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<tr><td colspan="6"><div id="space" align="center" />&nbsp;</td></tr>'));
		...
		$result .= '<td>RUC</td>';
		//$result .= '<td>Cliente</td>'; // solo para ULTRACOM esto va como comentario
			...
			foreach($results['documentos']['tipos'] as $td=>$paginas) {
				if ($td=='01'){
						...
						foreach($documentos['documentos'] as $numero=>$documento) {
				 			...
				 			//$result .= '<td>' . htmlentities($documento['cliente']) . '</td>'; //solo para ULTRACOM esto va como comentario
			...
			foreach($results['documentos']['tipos'] as $td=>$paginas) {
				if ($td=='03'){
						...
						foreach($documentos['documentos'] as $numero=>$documento) {
						    	...
						    	$result .= '<td>' . htmlentities($documento['documento']) . '</td>';
						    	$result .= '<td>' . htmlentities($documento['ruc']) . '</td>';
						    	//$result .= '<td>' . htmlentities($documento['cliente']) . '</td>';
						    	$result .= '<td>' . htmlentities($documento['razsocial']) . '</td>';
						    	$result .= '<td>' . htmlentities($documento['tc']) . '</td>';
						    	$result .= '<td>' . htmlentities($documento['neto']) . '</td>';
						    	$result .= '<td>' . htmlentities($documento['impuestos']) . '</td>';
		...
		$total = $results['documentos']['tipos']['03']['totales'];
	    	$result .= '<tr>';
	    	$result .= '<td colspan="14"><b>Sub-Total B/Venta </b></td>';
	    	$result .= '</tr><tr>';
		...
		$total = $results['documentos']['tipos']['07']['totales'];
	    	$result .= '<tr>';
	    	$result .= '<td colspan="14"><b>Sub-Total N/Credito </b></td>';
	    	$result .= '</tr><tr>';
	    	$result .= '<td colspan="7">&nbsp;</td>';
	    	$result .= '<td align="right">' . htmlentities($total['neto']) . '</td>';
	    	$result .= '<td align="right">' . htmlentities($total['impuestos']) . '</td>';
		...
		$total = $results['documentos']['tipos']['08']['totales'];
	    	$result .= '<tr>';
	    	$result .= '<td colspan="14"><b>Sub-Total N/Debito </b></td>';
	    	$result .= '</tr><tr>';
	    	$result .= '<td colspan="7">&nbsp;</td>';
	    	$result .= '<td align="right">' . htmlentities($total['neto']) . '</td>';
	    	$result .= '<td align="right">' . htmlentities($total['impuestos']) . '</td>';
		...
		foreach ($results['tickets_factura']['estaciones'] as $estacion=>$lineas) {
				...
				foreach($lineas['detalle'] as $i=>$linea) {
				   	...
				   	$result .= '<td>' . htmlentities($linea['cantidad']) . '</td>';
		    	...
		    	$result .= '<td>' . htmlentities($lineas['total_cantidad']) . '</td>';
		...
		$result .= '<td>' . htmlentities($results['tickets_factura']['total_cantidad']) . '</td>';		
		...
		$reporte->definirColumna("trans", $reporte->TIPO_TEXTO, 6, "L", "_anexo_tkt");
		...
		foreach($results['combustibles']['estaciones'] as $estacion=>$dias) {
	   		...
	   		if ($modo == "DETALLADO") {
				$v = VentasOficialModel::obtenerAlmaActual($estacion);
				$razsoc = $v[0];
				$ruc = $v[1];			

				$reporte->definirCabecera(2, "C", "Registro de Ventas - Centro de Costo: ".$estaciones[$estacion]." de ".$razsoc.", RUC: ".$ruc);
				$reporte->AddPage();
	    		}
		...
		$reporte->lineaH();
		$reporte->nuevaFila(array("rotulo"=>"Total General:"), "_rotulo");//TOTAL GENERAL SOLO TEXTO
		$reporte->nuevaFila($results['documentos']['totales'], "_market");//TOTAL GENERAL VALORES
		$reporte->lineaH();
		...
		foreach($results['tickets_factura']['estaciones'] as $es=>$estacion) {
				...
				foreach($estacion['detalle'] as $i=>$linea) {
					if($z <= 77) {
						$ValorNeto += $linea['vventa'];
						$Impuestos += $linea['igv'];
						$TotalVentas += $linea['importe'];
					}        
					if($z == 77) {
						$reporte->lineaH();
						$arr = array("razsocial"=>"Valor por Pagina ", "vventa"=>$ValorNeto, "igv"=>$Impuestos, "importe"=>$TotalVentas);
	...
	function listaSeries() {
		$series = VentasOficialModel::obtenerSeries();			
