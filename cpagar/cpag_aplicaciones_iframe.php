...
switch($accion){
	...
	case "completar_campos_cargo":
		...
		echo "cod_docu $cod_docu - num_docu $num_docu_cargo";
			$q = "select trim(cab.pro_cab_seriedocumento)||trim(cab.pro_cab_numdocumento) as num_documento
				, docref.desc_docu as desc_doc_ref			,	mone.desc_mone 
				, cab.pro_cab_tipdocreferencia as cod_doc_ref    
				, cab.pro_cab_numdocreferencia as num_doc_ref 
				, cab.pro_cab_imptotal as importe_inicial
				, to_char(cab.pro_cab_fecharegistro,'dd/mm/yyyy') as fecha_recepcion
				, to_char(cab.pro_cab_fechaemision,'dd/mm/yyyy') as fecha_venta
				, cab.pro_cab_impsaldo as saldo		    , pro.pro_rsocialbreve as proveedor_cargo
				, trim(cab.pro_codigo) as cod_proveedor	    
				...
				where cab.pro_cab_tipdocreferencia		=	docref.cod_docu
				and   cab.pro_cab_moneda				=	mone.cod_mone
				and   cab.pro_codigo					=	pro.pro_codigo
				and   cab.pro_cab_tipdocumento			=	'$cod_docu'
				and   trim(cab.pro_cab_seriedocumento)||trim(cab.pro_cab_numdocumento)='$num_docu'
			";
			echo $q;
			$rs = pg_exec($q);
			...
			if(pg_numrows($rs)>0){
				...
				parent.document.all('fila_proveedor_cargo').innerText = '".$A["cod_proveedor"]." - ".$A["proveedor_cargo"]."';
				parent.document.form1.c_cod_cliente.value='".$A["cod_proveedor"]."';
				...
				parent.document.all('fila_proveedor_cargo').innerText = '';
	...
	case "completar_campos_abono":
		...
		echo "cod_docu $cod_docu - num_docu $num_docu_cargo";
			...
			, cab.pro_cab_tipdocreferencia as cod_doc_ref    
			, cab.pro_cab_numdocreferencia as num_doc_ref 
			, cab.pro_cab_imptotal as importe_inicial, to_char(cab.pro_cab_fecharegistro,'dd/mm/yyyy') as fecha_recepcion
			, to_char(cab.pro_cab_fechaemision,'dd/mm/yyyy') as fecha_venta
			, cab.pro_cab_impsaldo as saldo
			, pro.pro_rsocialbreve as proveedor_cargo
			, trim(cab.pro_codigo) as cod_proveedor
			from cpag_ta_cabecera cab, 
			...
			if(pg_numrows($rs)>0){
				...
				if($A["cod_cliente"]==$cod_cliente){
					...
					parent.document.all('fila_proveedor_abono').innerText = '".$A["proveedor_cargo"]."';
					parent.document.form1.c_saldo_abono.value = '".$A["saldo"]."';
					</script>";
				}else{
					print "<script>alert('El documento $num_docu pertenece al cliente ".$A["cod_proveedor"]." y no al $cod_cliente');
					...
					parent.document.all('fila_proveedor_abono').innerText = '';
					</script>";
