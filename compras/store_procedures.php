...
function ayudaOrdCompraCosto($consulta , $condi , $like, $prove){
	$q = "SELECT util_fn_ayudas_ordcompras_costo('funccursor','$consulta','$condi','$like', '$prove');";
	//echo $q;
	pg_exec("BEGIN;");
	pg_exec($q);
	$rs = pg_exec("FETCH ALL IN funccursor;");
	pg_exec("COMMIT;");
	return $rs;
}
