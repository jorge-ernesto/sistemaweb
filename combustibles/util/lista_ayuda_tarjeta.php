<?php
//http://172.18.5.6/sistemaweb/combustibles/util/lista_ayuda_tarjeta.php?cod=Buscar.busquedatarjeta&des=Buscar.itemdescripcion&consulta=tarjetas
extract($_REQUEST);
  include('../../start.php');
  ...
  if($lista ==""){
    ...
    //if($Buscar=="Buscar"){
      if($busca=="")$busca = " ";
      $listado = TargpromocionModel::listarTarjetas(strtoupper(trim($busca)), '');
      $registro = $listado['datostarjeta'];
    //}
  }else{
    if($Buscar!="Buscar"){
      $objArticulo= TargpromocionModel::listarTarjetas(strtoupper(trim($busca)), '');
      $descripcion = isset($objArticulo[1]) ? $objArticulo[1] : null;

  ?>
  <script language="JavaScript">
    //opener.document.<?echo $des;?>.value = '<?php echo $descripcion; ?>';
    window.close();
    </script>
  <?php }else{
    if($busca=="")$busca = " ";
...
<form name="form1" method="post" action="">
  <table bgcolor="#FFFFFF" width="100%" border="0">
    <tr>
      <td height="17" colspan="4" class="form_cabecera">&nbsp;BUSCAR TARJETA </td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="4"><select name="lista" size="12">
        ...
        foreach($registro as $reg){
          echo "<option value='".$reg["nu_tarjeta_numero"]."'>".$reg["nu_tarjeta_numero"]." -- ".$reg["ch_tarjeta_descripcion"]."</option>";
        } 
  ?>
      </select></td>
    </tr>
