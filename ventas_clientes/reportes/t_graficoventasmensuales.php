...
class GraficoVentasMensualesTemplate extends Template
{
	...
	$estaciones = GraficoVentasMensualesModel::obtieneListaEstaciones();
	$estaciones['TODAS'] = "Todas las estaciones";

	$form = new form2("Grafico de Ventas Mensuales", "form_ventas_mensuales", FORM_METHOD_POST, "control.php", '', "control");
	$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden("rqst", "REPORTES.GRAFICOVENTASMENSUALES"));
	...
	$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<a href="javascript:show_calendar('."'form_ventas_mensuales.desde'".');"> <img src="/sistemaweb/images/showcalendar.gif" border="0" align="top"/></a>'));
	...
	$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<a href="javascript:show_calendar('."'form_ventas_mensuales.hasta'".');"> <img src="/sistemaweb/images/showcalendar.gif" border="0" align="top"/></a>'));
    ...
    function reporte($rs_mensual, $desde, $hasta, $estacion) {
		$total = 0;
		$maximo = 0;
		$minimo = 0;
		for($i=0;$i<pg_numrows($rs_mensual);$i++){ 
			$A = pg_fetch_array($rs_mensual,$i);    
			$total += $A[1];
			if ($maximo == 0)
				$minimo = $A[1];
			if ($A[1]>$maximo)
	...
	$reporte->definirCabecera(3, "C", "Grafico de Ventas Mensuales del " . $desde . " al " . $hasta);
