...
$tipoformu 		= ( isset($_REQUEST['tipoformu']) ? $_REQUEST['tipoformu'] : '' );
$c_formulario 	= ( isset($_REQUEST['c_formulario']) ? $_REQUEST['c_formulario'] : '' );
$fbuscar 		= ( isset($_REQUEST['fbuscar']) ? $_REQUEST['fbuscar'] : '' );
$boton 			= ( isset($_REQUEST['boton']) ? $_REQUEST['boton'] : '' );
$c_articulo 	= ( isset($_REQUEST['c_articulo']) ? $_REQUEST['c_articulo'] : '' );
$iAllProduct 	= ( isset($_REQUEST['iAllProduct']) ? $_REQUEST['iAllProduct'] : '' );

//Capturar valores para el UPDATE
$mov_fecha 		= ( isset($_REQUEST['mov_fecha']) ? $_REQUEST['mov_fecha'] : '');
$fecha_text2 	= ( isset($_REQUEST['fecha_text2']) ? $_REQUEST['fecha_text2'] : '');
$c_cantidad 	= ( isset($_REQUEST['c_cantidad']) ? $_REQUEST['c_cantidad'] : '');
$c_unitario 	= ( isset($_REQUEST['c_unitario']) ? $_REQUEST['c_unitario'] : '');
$c_costototal 	= ( isset($_REQUEST['c_costototal']) ? $_REQUEST['c_costototal'] : '');
$c_tipodoc 		= ( isset($_REQUEST['c_tipodoc']) ? $_REQUEST['c_tipodoc'] : '');
$c_docref 		= ( isset($_REQUEST['c_docref']) ? $_REQUEST['c_docref'] : '');

$iValidarAnularCompra = ( isset($_REQUEST['iValidarAnularCompra']) ? $_REQUEST['iValidarAnularCompra'] : '');
	...
	case "anularCompra":
		if ($iValidarAnularCompra == '0'){
			...
			SET
				...
				mov_usuario = '".$usuario."'
			WHERE 
				tran_codigo 	= '" . pg_escape_string($tipoformu) . "'
				AND mov_numero 	= '" . pg_escape_string($c_formulario) . "'
				AND mov_fecha::DATE	= '" . pg_escape_string($fe_emision) . "'
			";
			if ($sqlca->query($query) < 0 ){
...
<table bgcolor="#FFFFFF" cellspacing="0" cellpadding="2" border="0">
	...
	<tr>
		<td>Fecha Emision: </td>
		<td>
			<input maxlength="10" size="10" type='text' name ='fbuscar' id='fbuscar' class='fecha_formato' autocomplete="off" required value='<?php echo ( isset($_REQUEST['fbuscar']) ? $_REQUEST['fbuscar'] : '' ); ?>' <?php echo $onlyread; ?> /><span id="resultado"></span>
		</td>
    </tr>
    	<tr>
