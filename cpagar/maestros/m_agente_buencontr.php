...
Class AgenteBuencontrModel extends Model{

  function eliminarRegistros()
  {
    global $sqlca;
    if($sqlca->perform('sunat_buen_contribuyente  ', ' ', 'delete', "ch_ruc > '0'")>=0){
  ...
  {
    global $sqlca;
    if($sqlca->perform('sunat_buen_contribuyente', $datos, 'insert')>=0){
    } else 
    { 
  ...
  global $sqlca;
   ...
   if($ext=='txt' || $ext== 'csv' || $ext== 'zip')
   {
	...
	{//INICIO :: IF - Copiar el file al Servidor
	    ...
	    if(!empty($CampoArray))
	    {
                AgenteBuencontrModel::eliminarRegistros();
		...
		AgenteBuencontrModel::AgregarRegistros($datos[$key]);
	    }
