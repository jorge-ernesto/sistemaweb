...
try {
	...
	/*** Agregado 2020-01-27 ***/
	$_SESSION['nu_tarjeta_credito'] = trim($_POST['nu_tarjeta_credito']); /* Tarjeta de Credito */	
	/***/
	...
	if ($accion == "AsientosExcel") {
		...
		if ( $modulos==1 ) { //1=Ventas
			...
			$arrManualInvoiceSale = $objModel->getManualInvoiceSale( $arrParams, $postrans );
			// echo json_encode( $arrManualInvoiceSale ); //Agregado 2020-01-16
			// exit();									 //Agregado 2020-01-16
			...
			// echo json_encode( $arrPlayaInvoiceSale ); //Agregado 2020-01-16
			// exit();									//Agregado 2020-01-16
        	...
        	foreach($arrManualInvoiceSale["arrData"] as $row) {
        		...
        		if ( $iDetener == 0 && ($row["tipo"] != "03" || ($row["tipo"] == "03" && $row["total"] >= 700.00)) ){
					...
					$rows_["credito"]          = $row["credito"];          //Agregado 2020-01-16
					$rows_["fpago_referencia"] = $row["fpago_referencia"]; //Agregado 2020-01-18
					$rows_["at_referencia"]    = $row["at_referencia"];    //Agregado 2020-01-18
	                if ( count($rows_) > 0 )						
	            ...
	            if ( $sIDTipoDocumento != $row["tipo"] && $row["total"] < 700.00) {//BV INICIAL
	                if ( $row["tipo"] == "03" ){
						...
						$rows_bv["credito"]          = $row["credito"];          //Agregado 2020-01-16
						$rows_bv["fpago_referencia"] = $row["fpago_referencia"]; //Agregado 2020-01-18
						$rows_bv["at_referencia"]    = $row["at_referencia"];    //Agregado 2020-01-18
	                }
			...
			$arrManualInvoiceSaleReceivable = $objModel->getManualInvoiceSaleReceivable( $arrParams, $postrans );			
			// echo json_encode( $arrManualInvoiceSaleReceivable ); //Agregado 2020-01-20
			// exit();									             //Agregado 2020-01-20
			...
			// echo json_encode( $arrPlayaInvoiceSale ); //Agregado 2020-01-20
			// exit();									 //Agregado 2020-01-20
			...
			// echo json_encode( $arrPlayaInvoiceSaleExtornos ); //Agregado 2020-01-20
			// exit();									 //Agregado 2020-01-20
        	...
        	foreach($arrManualInvoiceSaleReceivable["arrData"] as $row) {
        		if ( $row["cuentacontable"] == "12121" ) {
	        		...
	        		if ( $iDetener == 0 && ($row["tipo"] != "03" || ($row["tipo"] == "03" && $row["total"] >= 700.00)) ){
						...
						$rows_["fpago"] = $row["fpago"]; //Agregado 2020-01-20
						$rows_["at"]    = $row["at"];    //Agregado 2020-01-20
		                if ( count($rows_) > 0 )
		                    $arrDataManualInvoiceSale[] = $rows_;
		                $sIDTipoDocumento = '';
	        		}
		            ...
		            if ( $sIDTipoDocumento != $row["tipo"] && $row["total"] < 700.00) {//BV INICIAL
		                if ( $row["tipo"] == "03" ){
							...
							$rows_bv["fpago"] = $row["fpago"]; //Agregado 2020-01-20
							$rows_bv["at"]    = $row["at"];    //Agregado 2020-01-20
		                }
		                $sIDTipoDocumento = $row["tipo"];
		                $sIDSerieDocumento = $row["serie"];
		            }
			...
			/*** Agregado 2020-02-07 ***/
			// echo json_encode( array($arrManualInvoiceSaleReceivable, $arrPlayaInvoiceSale, $arrPlayaInvoiceSaleExtornos, $arrDataManualInvoiceSale, $rows_bv, $arrDataManualInvoiceSaleCredit, $arrDataManualInvoiceSaleAccountBank, $arrInvoiceSale) );
			// exit();
			/***/
