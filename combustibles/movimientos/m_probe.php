...
class ProbeModel extends Model {
	...
	function obtenerAlmacenes($almacen) {
		...
		$sql = "SELECT
			...
			WHERE
			    	ch_clase_almacen='1'  
			    	AND ch_almacen = '".$_SESSION['almacen']."'
			ORDER BY
			    	ch_almacen;";
	...
	function validar($conf, $vec_check, $val, $almacen, $dia, $turno) {
		...
		for ($i=0; $i<count($val); $i++) {		
			if ($vec_check[$i] == "S" and $conf[$i] == "F" and $flag == 0) {
				...
				$dd = $d[1];
				$tt = $d[2]; 
				$rpta = ProbeModel::validaConsolidacion($dd, $tt);
				if($rpta == 1) {
					$flag = 1;
					return 0;
				}
	...
	function validaConsolidacion($dia, $turno) {
		global $sqlca;

		$sql = "SELECT 1 FROM pos_consolidacion WHERE dia='$dia' AND turno=$turno";
		if ($sqlca->query($sql) < 0) 
			return false;						
		$a = $sqlca->fetchRow();	
