function Ubicaciones() {
	...
	url = 'control.php?rqst=REPORTES.AJUSTEDOCUMENTOS&task=REGISTROS&action=Ubica&almacen='+almacen;
...
function Procesar(pregunta, f, cual, almacen, ubica, producto){
  	...
  	if(confirm(pregunta)){
		...
		url = 'control.php?rqst=REPORTES.AJUSTEDOCUMENTOS&action=fisico&task=PROCESANDO&almacen='+almacen+'&producto='+codigo+'&stkfisico='+fisico+'&ubica='+ubica;
		document.getElementById('control').src = url;
		return;
...
function Excel(almacen, producto, ubica){
	...
	url = 'control.php?rqst=REPORTES.AJUSTEDOCUMENTOS&action=reporte&task=EXCEL&almacen='+almacen+'&codigo='+producto+'&ubica='+ubica;
	document.getElementById('control').src = url;
	return;
