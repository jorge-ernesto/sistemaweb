...
class VarillasModel extends Model {
	...
	function Paginacion($pp, $pagina, $fecha, $fecha2, $almacen){
		global $sqlca;
		...
		for ($i = 0; $i < $sqlca->numrows(); $i++) {
			...
			$resultado[$i]['nu_costo_unitario'] 	= $a[19];
		}
	...
	function agregar($almacen,$hoy,$cnt_inicial,$cnt_final,$totcantidad,$totventa,$totabono,$totafericion,$clicredito,$clianticipo,$tarcredito,$descuentos,$faltantes,$sobrantes,$soles,$dolares,$surtidor_soles,$surtidor_m3,$mermas, $nu_costo_unitario){
		global $sqlca;
		...
		$validar = VarillasModel::ValidaRegistro($almacen, $fecha);
		...
		settype($nu_costo_unitario,"double");

		if ($validar == 1){
	
			$query2 = "
				INSERT INTO comb_liquidaciongnv(
					ch_almacen,          
					dt_fecha,          
					contometro_inicial,
					contometro_final,
	...
	function ActualizarRegistro($almacen,$fecha,$cnt_inicial,$cnt_final,$totcantidad,$totventa,$totabono,$totafericion,$clicredito,$clianticipo,$tarcredito,$descuentos,$faltantes,$sobrantes,$soles,$dolares,$surtidor_soles,$surtidor_m3,$mermas, $nu_costo_unitario){
		global $sqlca;
		...
		SET
			...
			mermas_m3 			= $mermas,
			nu_costo_unitario 	= $nu_costo_unitario
		WHERE 
			ch_almacen 		= '$almacen'
			AND dt_fecha 	= '$fecha';
		";
	...
	function recuperarRegistroArray($almacen,$fecha){
		...
		WHERE
			ch_almacen 		= '$almacen'
			AND dt_fecha 	= to_date('$fecha','DD/MM/YYYY')
		";
