...
<script languaje="JavaScript">
	location.href='inv_add_guia.php?fm=09&flg=A';
</script>
<?php
...
?>
<script languaje="JavaScript">
	location.href='inv_add_guia.php?fm=<?php echo $fm;?>&nromov=<?php echo $nform;?>&flg=A';
</script>
<?php }else{  ?>
<script languaje="JavaScript">
...
$sqladd=" and g.dt_guia_fecha between '$fecini' and '$fecfin' ";
//echo $sqladd;
...
{
	$sql2="select g.mov_numero , to_char(g.dt_guia_fecha,'dd/mm/yyyy hh24:mi:ss') as mov_fecha
	,'09' as mov_tipdocuref, g.ch_guia_numero as mov_docurefe  
	,g.mov_almaorigen , g.mov_almadestino
	,g.mov_almacen , g.art_codigo ||' - '|| g.ch_codigo_activo
	,g.nu_guia_cantidad , g.nu_costounitario 
	,a.art_descripcion , g.tran_codigo
	from inv_ta_guias_remision g,int_articulos a
	where g.art_codigo=a.art_codigo 
 	".$sqladd." ".$xlim." ";
	//echo $sql2;	
	$flg = "V";
...
MOVIMIENTOS DE GUIAS<?php echo $descform; ?>
<hr noshade>
...
<form action="" method="post" name="formular">
  <table border="1">
	...
	&nbsp;<a href="javascript:show_calendar('formular.diasd');"><img src="/sistemaweb/images/showcalendar.gif" border="0" align="top"/></a><div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"/></div></th>
      <th>HASTA</th>
      <th><input type="text" name="diasa" size="10" value="<?php echo $diaa.'/'.$mesa.'/'.$anoa ?>" readonly="true"/>
	&nbsp;<a href="javascript:show_calendar('formular.diasa');"><img src="/sistemaweb/images/showcalendar.gif" border="0" align="top"/></a></th>
...
from inv_ta_guias_remision g,int_articulos a
where g.art_codigo=a.art_codigo 
 ".$sqladd." order by g.dt_guia_fecha desc  ".$bddsql."  ";
...
if($ilimit2>0) {
	while($irow2<$ilimit2) {
		...
		$tran_codigo = pg_result($xsql2,$irow2,11); 
...
?>
	<tr bgcolor="#CCCC99" onMouseOver="this.style.backgroundColor='#FFFFCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#CCCC99'"o"];" onClick="mostrarCintilloGuias('<?php echo $a0;?>','<?php echo $tran_codigo;?>');">
<?php
			echo "<td>&nbsp;<input type='radio' name='nform' value='".$a0."'></td>";
//			echo "<tr><td>&nbsp;<input type='radio' name='nform' value='".$a0."' onClick='submit(this)'></td>";
		}
		$a7 = str_replace("- null","",$a7);
		//$a7 = str_replace("null -","",$a7);
        echo "<td>&nbsp;".$a0."</td><td>&nbsp;".$a1."</td><td>".$a12."</td><td>&nbsp;".$a2." - ".$a3."</td>";
        echo "<td>".$a4."</td><td>&nbsp;".$a5."</td><td>&nbsp;".$a6."</td>";
		echo "<td>".$a7."-.".$tran_codigo."</td><td><p align='right'>".$a8."</p></td><td><p align='right'>".$a9."</p></td><td>".$a10."</td></tr>";
		$irow2++;
	}
}
?>
