...
function confirmarLink(pregunta, accionY, accionN, target){
  if(confirm(pregunta))
   ...
   document.forms[0].action= "control.php?rqst=PROMOCIONES.PUNTOSXPRODUCTO&task=PUNTOSXPRODUCTO";
   }
}
...
function regresar(){
	url = 'control.php?rqst=PROMOCIONES.PUNTOSXPRODUCTO&task=PUNTOSXPRODUCTO';
...
{
   //rxp = rxp.value;
   urlPagina = 'control.php?rqst=PROMOCIONES.PUNTOSXPRODUCTO&task=PUNTOSXPRODUCTO&rxp='+rxp+'&pagina='+valor;
...
function validar_registro_horariomulti(){
	...
	txtFactorMulti =document.getElementsByName('horamultifactor')[0];
		...
		if(idCampania.value==''){
			...
			alert('�Ingrese cantidad de D�as de Multiplicaci�n!');
			return false;
		}
			...
			alert('�Ingrese Factor de multiplicaci�n!');
			return false;
		}
...
function volver_atras(){
	urlPagina = 'control.php?rqst=PROMOCIONES.PUNTOSXPRODUCTO&task=PUNTOSXPRODUCTO';
    document.getElementById('control').src = urlPagina;
}
