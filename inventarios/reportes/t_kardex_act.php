...
class KardexActTemplate extends Template {
    ...
    function Titulo() {
        return '<h2 align="center" style="color:#336699;"><b>Kardex</b></h2>';
    }
    ...
    function formSearch() {
        ...
        $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<tr><td>Fecha Inicio: </td><td>:</td><td>'));
        ...
        $form->addElement(FORM_GROUP_MAIN, new form_element_anytext('<tr><td align="left">(Opcional) Filtar por<td>:'));
...
*/
        ...
        <tr>
            ...
            <div id="label2" style="display:block;">
                <input type="hidden" id="txt-Nu_Id_Producto" name="art_desde" placeholder="Ingresar codigo producto" value="" maxlength="25" size="25">
                <input type="text" id="txt-No_Producto" onkeyup="autocompleteBridge(0)" class="mayuscula" name="art_desde2" placeholder="Ingresar Código o Nombre" autocomplete="off" value="" maxlength="35" size="35">  
        '));
/*
...
*/
        $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('
        <tr><td><div id="l" style="visibility:hidden;">L&iacute;nea</td><td><div id="l2" style="visibility:hidden;">:</td><td>
            <input type="hidden" id="txt-Nu_Id_Linea" name="art_linea" placeholder="Ingresar codigo producto" value="" maxlength="25" size="25">
            <input type="text" style="visibility:hidden;" id="txt-No_Linea" class="mayuscula" name="art_linea2" placeholder="Ingresar Código o Nombre" autocomplete="off" value="" maxlength="35" size="35">
        '));

        $form->addElement(FORM_GROUP_MAIN, new form_element_anytext('<tr><td align="left">Tipo Vista<td>: '));
        $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<td colspan="2" align="left">'));
	    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<select name="tipovista" size="1">
    ...
    function listado($resultado, $resulta, $resta, $desde, $hasta, $art_desde, $estacion, $tipo, $linea) {
        ...
        foreach ($resultado['almacenes'] as $mov_almacen => $almacen) {//#FFFFFF bordercolor="#FFFFFF" FFFFFF
            ...
            $result .= '<td rowspan="3" align="center" bgcolor="#9DD2DA" style="color:#126775;font-size:11px;"><font size=1><b>Entidad</td>';
            $result .= '<td rowspan="3" align="center" bgcolor="#9DD2DA" style="color:#126775;font-size:11px;"><font size=1><b>Liquidación</td>';
            $result .= '</tr>';
            ...
            foreach ($almacen['articulos'] as $art_codigo => $articulo) {
                ...
                foreach ($articulo['movimientos'] as $i => $movimiento) {
                    ...
                    if ($tipo == "CONTABLE") {
                        $result .= '<td bgcolor="#EBEDED" align = "right">' . htmlentities(number_format($movimiento['mov_cant_actual'], 4, '.', ',')) . '</td>'; //AQUI ESTA EL PROBLEMA
                        $result .= '<td bgcolor="#EBEDED" align = "right">' . htmlentities(number_format($movimiento['mov_val_unit_act'], 6, '.', ',')) . '</td>'; //AQUI ESTA EL PROBLEMA
                        $result .= '<td bgcolor="#EBEDED" align = "right">' . htmlentities(number_format($movimiento['mov_total_act'], 4, '.', ',')) . '</td>'; //AQUI ESTA EL PROBLEMA
                    } else {
                        $result .= '<td bgcolor="#EBEDED" align = "right">' . htmlentities(number_format($movimiento['mov_cant_actual'], 4, '.', ',')) . '</td>';
                    }
                    $result .= '<td bgcolor="#EBEDED">' . htmlentities($movimiento['sNombreEntidad']) . '</td>';
                    $result .= '<td bgcolor="#EBEDED">' . htmlentities($movimiento['sDocumentoLiquidado']) . '</td>';
                }
...
//        $reporte->definirCabecera(4, "L", "RUC: " . $empresa['ruc']);
		...
		if ($tipo == "CONTABLE") {
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $ini . ':Q' . $ini)->applyFromArray($cabecera);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $ini . ':Q' . $ini)->getFont()->setBold(true);
                    $objPHPExcel->setActiveSheetIndex($hoja)
                        ...
                        ->setCellValue('P' . $ini, 'ENTIDAD')
                        ->setCellValue('Q' . $ini, 'LIQUIDACIÓN');
			...
			$unit_inicial = (!empty($articulo['saldoinicial']['unit_anterior'])) ? (float) $articulo['saldoinicial']['unit_anterior'] : 0;
                ...
                foreach ($articulo['movimientos'] as $i => $movimiento) {
                    ...
                    if ($tipo == "CONTABLE") {
                        ...
                        $sNombreEntidad = (!empty($movimiento['sNombreEntidad'])) ? $movimiento['sNombreEntidad'] : '';
                        $sDocumentoLiquidado = (!empty($movimiento['sDocumentoLiquidado'])) ? $movimiento['sDocumentoLiquidado'] : '';
                        ...
                        $objPHPExcel->setActiveSheetIndex($hoja)->setCellValue('F' . $ini, $movimiento['tran_codigo'])
                                ...
                                ->setCellValue('P' . $ini, $sNombreEntidad)
                                ->setCellValue('Q' . $ini, $sDocumentoLiquidado);
                    } else {
