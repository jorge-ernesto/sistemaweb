...
if($_POST) {
	...
	$newcapac = $_POST['newcapac'];
	$newultlect = $_POST['newultlect'];

	$newcapac = (int)$newcapac;
}

//var_dump($_SESSION);
$user = isset($user) ? $user : $_SESSION["auth_usuario"];
 ...
 switch ($boton) {
		...
		if(strlen($newcod)>0) {
			if ($newultlect != '') {
				if($newcapac > 0) {
					...
					?><script>alert("Campo CAPACIDAD debe ser Numerico y Mayor que cero")</script><?php
				}
			} else {
				?><script>alert("Alerta: Campo ULT LECT GAL vacio.")</script><?php
			}
		} else {
			?><script>alert("Debe ingresar un Codigo valido")</script><?php
		}
      break;
   case Modificar:
   ...
   case Eliminar:
		...
		while($irowm<$ilimitm) {
			...
			//if($idm[$am0]==$idp[$am0]) {
			if ($am0==$idp[$am0]) {
				$sqlupd=" delete from comb_ta_tanques where ch_tanque='".$am0."' and ch_sucursal=trim('$cod_almacen') ";
				$xsqlupd=pg_exec($coneccion,$sqlupd);
				//echo 'query DEL: ';var_dump($xsqlupd);
			}
			$irowm++;
		}
...
<body>
<h2 style="color:#336699;" align="center">CONFIGURACION DE TANQUES</td>
<p>
</p>
<form action='m_tanques.php' method='post' name="form1">
	Almacen:
  ...
  <tr>
		...
		<th class="grid_cabecera" width="50">CODIGO</th>
		<th class="grid_cabecera" width="100">PRODUCTO</th>
		<th class="grid_cabecera" width="110">CAPACIDAD</th>
		<th class="grid_cabecera" width="110">ULT LECT GAL</th>
		<th class="grid_cabecera" width="110"></th>
  </tr>
...
?>    
      ...
      <td><button name="boton" type="submit" value="Agregar" onClick="javascript:cambiarAction(form1,'Agregar')"><img src="/sistemaweb/icons/agregar.gif" align="right" />Agregar</button>
     </td>
  ...
  <tr>
		...
		<th class="grid_cabecera" width="110">CAPACIDAD</th>
		<th class="grid_cabecera" width="110">ULT LECT GAL</th>
		<th class="grid_cabecera" width="110">FECHA ULT LECT</th>
  </tr>
<?php
$irow=0;
$sql=" select ch_tanque,ch_codigocombustible,nu_capacidad,nu_ultimamedida,dt_fechaultimamedida
