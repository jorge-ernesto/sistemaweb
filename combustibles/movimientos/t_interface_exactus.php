<?php
class InterfaceExactusTemplate extends Template {
	function titulo() {
		$titulo = '<div align="center"><h2>Interface Opensoft --> Exactus</h2></div><hr>';
		return $titulo;
	}
	...
	function formInterfaceExactus($datos,$CbSucursales,$Parametros) {
		$CbMes = InterfaceExactusTemplate::ListadoMes();
		...
		$form = new form2('INTERFACE EXACTUS', 'form_agen_ret', FORM_METHOD_POST, 'control.php', '', 'control');
		$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MOVIMIENTOS.INTERFAZEXACTUS'));
		$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'INTERFAZEXACTUS'));
	...
	function imprimeResultado($res) {
		$result = "<p style=\"text-align: center;\">" . (($res===TRUE)?"Se ha copiado la informaci&oacute;n al Exactus":"No se pudo copiar la informaci&oacute;n al Exactus") . "</p>";
		return $result;
	}
	...
	function imprimeConsulta($resultados,$inicio,$fin,$sucursal){
		...
		for ($i = 0; $i < count($resultados); $i++) {
			...
			//$result .= '<td align="center" ><a href="javascript:confirmarLink(\'�Desea eliminar el proceso de fecha '.trim($a["inicio"]).'?\',\'control.php?rqst=MOVIMIENTOS.INTERFAZEXACTUS&task=INTERFAZEXACTUS&action=Eliminar&id='.trim($a["id"]).'\')"><img alt="Eliminar Proceso" title="Eliminar Proceso" src="/sistemaweb/icons/delete16x16.png" align="middle" border="0"/></a>&nbsp;</td>';
			$result .= '</tr>';
		}
		$result .= '</table>';
