...
class CierreActivoPasivoModel {
	...
	function getAsientoCierreActivoPasivo($data, $objHelper) {
		...
		$arrParams['Fe_Periodo']            = $Fe_Periodo;
		$arrParams['tca_fecha']             = $Fe_Periodo . "-12-31";

		try {
			//GENERAMOS ASIENTOS DE REVERSION 
			$response = $this->generarAsientosReversion($arrParams);			
	...
	function saveAsientoCierreActivoPasivo($response, $objHelper) {
		$this->executeInsert($response['entry']);		
	}

	function deleteAsientoCierreActivoPasivo($data, $objHelper) {
		// echo "<script>console.log('data')</script>";
		// echo "<script>console.log('" . json_encode($data) . "')</script>";
		...
		$this->eliminarAsientosReversion($arrParams);
	}

	function regenerateBalance($data, $objHelper) {
		// echo "<script>console.log('data')</script>";
		// echo "<script>console.log('" . json_encode($data) . "')</script>";
		...
		//GENERAMOS BALANCE CONTABLE DE TODOS LOS MESES DEL AÑO
		$objHelper->generarBalancePorAnioCierre($arrParams);
	}

	//FUNCIONES ADICIONALES
	function generarAsientosReversion($arrParams) {
		global $sqlca;

		$Nu_Almacen            = $arrParams['Nu_Almacen'];
		$SubDiario             = $arrParams['SubDiario'];
		$Descripcion_Subdiario = $arrParams['Descripcion_Subdiario'];
		$Fe_Periodo            = $arrParams['Fe_Periodo'];
		...
		$sql_data_asiento = "
			...
			WHERE 
				1 = 1
				AND TO_CHAR(DATE(b.dateacct),'YYYY') = '" . $Fe_Periodo . "'
		...
		$arrEntry = $this->formatoAsientosReversion($arrData, $arrTipoCambio, $arrParams);
	...
	function formatoAsientosReversion($arrData, $arrTipoCambio, $arrParams) {
		//GENERAMOS ASIENTOS DE REVERSION
		$act_entryline = array();
		...
		foreach ($arrData as $key => $value) {
			...
			$amtdt = $haber; //amtdt normalmente es el debe, pero le ponemos haber, invertimos
			$amtct = $debe;  //amtct normalmente es el haber, pero le ponemos debe, invertimos
			...
			$regid            = "-";
			$tab_currency     = "01";
			$description      = "ASIENTO DE REVERSION";
	...
	function eliminarAsientosReversion($arrParams) {
		global $sqlca;
		...
		$sql_act_entryline = "
			DELETE FROM act_entryline WHERE act_entry_id IN (SELECT 
															...
															WHERE 
																...
																AND DATE(documentdate) = '$Fe_Periodo-12-31'
																AND act_entrytype_id IN ('11'));
		";
