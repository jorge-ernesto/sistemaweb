...
class AplicacionesTemplate extends Template {
	function titulo() {
		$titulo = '<div align="center" style="color:#336699"><h2>Aplicaciones - Fecha Aplicacion: '.$_SESSION['fec_aplicacion'].'</h2></div>';
		return $titulo;
	}
	...
	function formInterfaz() {
		...
		$form->addElement(FORM_GROUP_MAIN, new f2element_text ('fecha','Fecha :', date('d/m/Y'), espacios(2), 10, 10,array(), ''));
		//$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<a href="javascript:show_calendar('."'Interfaz.fecha'".');"><img src="/sistemaweb/images/showcalendar.gif"></a>&nbsp;&nbsp;'));
	...
	function listadoCargos($registros) {
		...
		$titulo_grid = '<h4 align="center" style="color:#336699"><b> APLICACIONES DE CLIENTES </b></h4>';
		//formulario de busqueda
		$columnas = array('COD. CLIE.','RAZON SOCIAL', 'COD. TIPO', 'NOM. TIPO','SERIE','NUMERO', 'FECHA EMISION', 'FECHA SALDO', 'MONEDA','TOTAL','SALDO');
						...
						for($i=0;$i<count($columnas);$i++)
							...
							foreach($registros as $reg) {
								...
								for ($i=0; $i < count($columnas); $i++)
									...
									$listado .= '<td><A href="control.php?rqst=MOVIMIENTOS.APLICACIONES&task=APLICACIONES&action=Aplicacion&clicodigo=' . trim($reg['cli_codigo']) . '&registroid='.trim($regCod).'&busqueda[codigo]='.$_REQUEST['busqueda']['codigo'].'&tipo='.trim($reg["ch_tipdocumento"]).'&monedaza='.$auximon.'" target="control"><img src="/sistemaweb/icons/open.gif" alt="Editar" align="middle" border="0"/></A></td>';
									$listado .= '</tr>';
								$counter++;
							}
