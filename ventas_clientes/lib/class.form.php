...
class form_element_checkbox extends form_element {
  ...
  var $size;
  var $values;
  function __construct ($title, $name, $value, $separator, $style, $bChecked = false) {
    parent::__construct($title, $name, $value, $separator);
    if ($style != '') $this->style = $style;
    $this->size = $size;
    $this->values = $values;
  ...
  function getTag () {
    $r = '';
    $r .= '<span class="'.$this->style.'">';
    //$r .= '<input type=checkbox name="'.$this->name.'" value="1"'.($this->value? ' checked':'').'>'.$title;
    $r .= '<input type=checkbox name="'.$this->name.'" value="' . $this->value . '"' .($this->checked? ' checked':'').'>'.$title;
    $r .= '</span>';
