...
//METODO QUE RETORNA UN MENSAJE DE ERROR
  function errorResultado($errormsg){
    ...
    $columnas = array('CAMPAÑA','COD. ARTICULO','DESCRIPCI�N','FEC. CREACION','FEC. VENCIMIENTO','OBSERVACION','PUNTOS DE CANJE');
...
for($i=0;$i<count($columnas);$i++)
	...
	$listado .='<td class="'.$color.'">'.$reg["nu_item_puntos"].'&nbsp;</td>';
      ...
      $listado .= '<td class="'.$color.'"> <A href="control.php?rqst=PROMOCIONES.CANJEITEM&task=CANJEITEM&action=Modificar&itemid='.$reg["id_item"].'&articulocod='.$reg["art_codigo"].'&itemdescripcion='.$reg["ch_item_descripcion"].'&itemfechacre='.$reg["dt_item_fecha_creacion"].'&itemfechaven='.$reg["dt_item_fecha_vencimiento"].'&itemobservacion='.$reg["ch_item_observacion"].'&itempuntos='.$reg['nu_item_puntos'].'&id_campana='.$reg['id_campana'].'&ch_campana_descripcion='.$reg['ch_campana_descripcion'].'" target="control"><img alt="Editar Producto" title="Editar Producto" src="/sistemaweb/icons/editar.gif" align="middle" border="0"/></A>&nbsp;</td>';
	


		$listado .= '<td class="'.$color.'" ><a href="javascript:confirmarLink(\'�Desea eliminar el producto N� '.
		$reg['art_codigo'].' - '.$reg['ch_item_descripcion'] .'?\',\'control.php?rqst=PROMOCIONES.CANJEITEM&task=CANJEITEM&action=Eliminar&itemid='.
		$reg["id_item"].'\',\'control\')"><img alt="Eliminar Producto" title="Eliminar Producto" src="/sistemaweb/icons/delete16x16.png" align="middle" 		
					border="0"/></a></td>';	
 ...
 function formBuscar($paginacion){
   echo "ENTRO BUSCAR\n";
    ...
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'PROMOCIONES.CANJEITEM'));
   ...
   echo ' entro a formProducto';
	...
	$form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('id_campana', @$item["id_campana"]));
	...
	if($_REQUEST['action'] == 'Modificar'){
			...
			$form->addElement(FORM_GROUP_MAIN, new f2element_text ('ch_campana_descripcion','CAMPA&Ntilde;A *  </td><td colspan="2" class="form_texto">: ', trim(@$item["ch_campana_descripcion"]),'', 50, 13,'',array("readonly")));

//$form->addElement(FORM_GROUP_MAIN, new f2element_hidden ('id_campana','',   trim(@$item["id_campana"]),'', 15, 15,'',array("readonly")));
			
			//$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<img src="../images/help.gif" style="cursor:hand" width="16" height="15" onClick="javascript:mostrarAyuda('."'../combustibles/util/lista_ayuda_campaniafide.php','form_itemcanje.id_campana','form_itemcanje.ch_campana_descripcion','campanias'".')">&iquest;Necesita ayuda?'));
			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr>'));
	} else {
			$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<tr><td class="form_texto">'));
			
			$form->addElement(FORM_GROUP_MAIN, new f2element_text ('ch_campana_descripcion','CAMPA&Ntilde;A *  </td><td colspan="2" class="form_texto">: ','','', 50, 13,'',array("readonly")));
	...
	$form->addElement(FORM_GROUP_MAIN, new f2element_text ('itemarticulo','ART&Iacute;CULO *  </td><td colspan="2" class="form_texto">: ', 
	trim(@$item["art_codigo"]),'', 25, 13,'',array()));
	
	 $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<img src="../images/help.gif" style="cursor:hand" width="16" height="15" onClick="javascript:mostrarAyuda('."'../combustibles/util/lista_ayuda.php','form_itemcanje.itemarticulo','form_itemcanje.itemdescripcion','articulos'".')">&iquest;Necesita ayuda?'));
	//$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<img src="/sistemaweb/icons/search20x20.gif">'));

    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td class="form_texto">'));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('itemdescripcion','DESCRIPCI&Oacute;N * </td><td colspan="2" class="form_texto">: ', 
	trim(@$item["ch_item_descripcion"]),'', 50, 40,'',array()));
	$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td class="form_texto">'));
	
	if($_REQUEST['action'] == 'Modificar'){
		$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('FECHA CREACI&Oacute;N </td><td class="form_texto">: &nbsp;<span class="form_valor_texto">'
	...
	$form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td class="form_texto">'));
	 $form->addElement(FORM_GROUP_MAIN, new f2element_textarea  ('itemobservacion','OBSERVACI&Oacute;N </td><td colspan="2" class="form_texto">:</td></tr>	
