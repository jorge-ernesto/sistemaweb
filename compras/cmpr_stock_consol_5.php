<?php
			...
			pg_exec($conector_id,"select calcula_reqpend_rango_fecha('$v_almacen','$v_linea','".$funcion->date_format($v_fecha_inicial,'YYYY-MM-DD')."','".$funcion->date_format($v_fecha_final,'YYYY-MM-DD')."', 'recordset')");
			// echo "select calcula_reqpend_rango_fecha('$v_almacen','$v_linea','".$funcion->date_format($v_fecha_inicial,'YYYY-MM-DD')."','".$funcion->date_format($v_fecha_final,'YYYY-MM-DD')."', 'recordset')";
			...
			for($i=0;$i<pg_numrows($v_rs);$i++){
				...
				$v_art_codigo_requerimiento=pg_result($v_rs,$i,1);
				...
				$v_sqlareq_det="update COM_TA_AREQ_DETALLE 
								set NU_AREQ_CANTIDAD_REQ_ORI=$v_art_codigo_requerimiento,
								NU_AREQ_CANTIDAD_REQ_APRO=$v_art_codigo_requerimiento 
