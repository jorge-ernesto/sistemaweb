...
class InterfaceMovTemplateCE extends Template {
    ...
    function formInterfaceMov($datos) {
        ...
        $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MOVIMIENTOS.INTERFAZCE'));
        $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'INTERFAZCE'));
		...
		if (count($resultado_postrans) > 0) {
				...
				if (empty($array_index_global[$value["id"]])) {
		        	...
		        	$objPHPExcel->setActiveSheetIndex($hoja)
		                ...
		                ->setCellValue('H' . $bucle, $value["moneda"])
		                ->setCellValue('I' . $bucle, $value["sede_venta"])
			        ...
			        $objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow(9, $bucle, $value["alamcen"], PHPExcel_Cell_DataType::TYPE_STRING);
