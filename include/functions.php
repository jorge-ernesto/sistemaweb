...
function valida_existe_art($coneccion,$newcodart) {
        ...
        }else{ $f_valart=" C�igo de art�ulo "; }
}
...
function valida_prov($coneccion,$updprov1) {
        ...
        if(pg_numrows($xsqlval_prov)>0){ $f_prov=""; } else { $f_prov=" C�igo del proveedor "; }
}
...
function valida_almd($coneccion,$updalmacd1) {
        ...
        if(pg_numrows($xsqlval_almd)>0) { $f_almd=""; }else{ $f_almd=" Almac� de destino "; }
}
...
function valida_fecha($dia,$mes,$ano) {
        ...
        if(checkdate($mes,$dia,$ano)) { $mens_valida_fecha=""; } else { $mens_valida_fecha=" La fecha ".$dia."/".$mes."/".$ano." no es v�ida !!!"; }
}
...
function dame_estilo($dia_imprimir){
        ...
        //dependiendo si el d� es Hoy, Domigo o Cualquier otro, devuelvo un estilo
...
function mostrar_calendario($mes,$ano){
        ...
        //recorro todos los dem� d�s hasta el final del mes
        $numero_dia = 0;
        while ($dia_actual <= $ultimo_dia){
                ...
                $dia_actual++;
                $numero_dia++;
                //si es el u�imo de la semana, me pongo al principio de la semana y escribo el </tr>
                if ($numero_dia == 7){
                        $numero_dia = 0;
                        echo "</tr>";
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Funci� que escribe en la p�ina un fomrulario preparado para introducir una fecha y enlazado con el calendario para seleccionarla comodamente
////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function sobrantesyfaltantesReporte($almacen,$cod_tanque,$fechad,$fechaa){
//Aqui se saca el codigo del combustible de una vez para no estar metiendo mas cosas
$sqlcomb = "SELECT ch_codigocombustible 
            FROM comb_ta_tanques 
            WHERE ch_tanque='$cod_tanque'
            AND ch_sucursal=trim('$almacen')";
//echo "<!--QUERYC : $sqlcomb-->\n";
$comb = pg_exec($sqlcomb);
...
//SALDO
        $qe =  "SELECT to_char(dt_fechamedicion,'DD-MM-YYYY') AS fecha,
                        sum(nu_medicion) AS saldo
                FROM comb_ta_mediciondiaria
                WHERE ch_sucursal=trim('$almacen')
       ...
       // echo "<!--SALDO : ".$qe."\n-->";
                ...
                for($i=0;$i<count($fec_saldo);$i++){
                        ...
                        for($a=0;$a<count($Fe);$a++){
                                if($Fe[$a]==$fec_saldo[$i]){  $rep[$i][1] = $Saldo[$a];  }
                        }
                }
...
//COMPRA
$limit = count($fec)-1;
$sqlrs1 = "SELECT to_char(mov.mov_fecha,'DD-MM-YYYY') AS fecha,
                  sum(mov.mov_cantidad) AS compra 
           FROM inv_movialma mov 
           WHERE tran_codigo='21'
           AND mov.mov_almacen=trim('$almacen')
	   AND mov.art_codigo='$cod_combustible'
	   AND to_date(to_char(mov.mov_fecha,'DD-MM-YYYY'),'DD-MM-YYYY') >= to_date('$fec[0]','dd-mm-yyyy')
	   AND to_date(to_char(mov.mov_fecha,'DD-MM-YYYY'),'DD-MM-YYYY') <= to_date('$fec[$limit]','dd-mm-yyyy')
	   GROUP BY fecha";
//echo "<!--COMPRA : ".$sqlrs1."\n-->";
$rs1 = pg_exec($sqlrs1);
...
//AFERICION
    $sqlafer = "SELECT to_char(cont.dt_fechaparte,'DD-MM-YYYY') AS fecha, 
                       sum(cont.nu_afericionveces_x_5*5) AS medicion 
                FROM comb_ta_contometros cont
		WHERE cont.ch_sucursal=trim('$almacen')
...
//VENTA
        $sqlven = "SELECT to_char(dt_fechaparte,'DD-MM-YYYY') AS fecha,
                          sum(cont.nu_ventagalon) AS venta
		   FROM comb_ta_contometros cont
		   WHERE cont.ch_sucursal=trim('$almacen')
		   AND dt_fechaparte >= to_date('$fechad','dd-mm-yyyy')
		   AND dt_fechaparte <= to_date('$fechaa','dd-mm-yyyy')
    ...
    //echo "<!--VENTAS : ".$sqlven."\n-->";
        ...
        for($i=0;$i<count($fec);$i++){
                ...
                if($F4[$b]==$fec[$i]){  $rep[$i][4] = $VENTA[$b];  }
        }

//INGRESO
        $sqlingr = "select to_char(mov_fecha,'DD-MM-YYYY') as fecha,sum(mov_cantidad) as compra
        from inv_movialma
        where mov_almacen=trim('$almacen')
        ...
        and tran_codigo='27' group by fecha";
        //echo "<!--INGRESO : ".$sqlingr."\n-->";
        $rs1 = pg_exec($sqlingr);
        for($a=0;$a<pg_numrows($rs1);$a++){
                $A = pg_fetch_row($rs1,$a);
                $F5[$a] = $A[0];
...
//SALIDA
        $rs1 = pg_exec("select to_char(mov_fecha,'DD-MM-YYYY') as fecha,sum(mov_cantidad) as compra
        from inv_movialma
        where mov_almacen=trim('$almacen')
        and art_codigo='$cod_combustible'
        and to_date(to_char(mov_fecha,'DD-MM-YYYY'),'DD-MM-YYYY') >= to_date('$fec[0]','DD-MM-YYYY')
        and to_date(to_char(mov_fecha,'DD-MM-YYYY'),'DD-MM-YYYY') <= to_date('$fec[$limit]','DD-MM-YYYY')
        and tran_codigo='28' group by fecha");
        for($a=0;$a<pg_numrows($rs1);$a++){
                $A = pg_fetch_row($rs1,$a);
                $F6[$a] = $A[0];
                $SAL[$a] = $A[1];
...
//VARILLA
        $rs1 = pg_exec("select to_char(dt_fechamedicion,'DD-MM-YYYY') as fecha,sum(nu_medicion) as saldo
        from comb_ta_mediciondiaria
        where ch_sucursal=trim('$almacen')
        and dt_fechamedicion >= to_date('$fechad','dd-mm-yyyy')
        and dt_fechamedicion <= to_date('$fechaa','dd-mm-yyyy')
...
//FIN DE REPORTE DE SOBRANTES Y FALTANTES
