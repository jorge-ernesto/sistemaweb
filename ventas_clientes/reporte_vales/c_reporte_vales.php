...
class LiquidacionValesController extends Controller {
	function AgruparRegistoFacturaNormal($rsdata,$sucursal) {
		...
		foreach ($rsdata as $value) {
			...
			$factura_procesada[$cli_anticipo][$ch_cliente][$sucursal][$art_codigo]['cantidad']+=$value['nu_cantidad'];
	...
	function AgruparRegistoFacturaXPlaca($rsdata) {
		try {
			...
			foreach ($rsdata as $value) {
				...
				$factura_procesada[$cli_anticipo][$ch_cliente][$ch_sucursal][$ch_placa][$art_codigo]['cantidad']+=$value['nu_cantidad'];
	...
	function AgruparRegistoFacturaXNotaDespacho($rsdata) {
		try {
			...
			foreach ($rsdata as $value) {
				...
				$factura_procesada[$cli_anticipo][$ch_cliente][$ch_sucursal][$ch_documento][$art_codigo]['cantidad']+=$value['nu_cantidad'];
	...
	function AgruparRegistoFacturaXProducto($rsdata) {
		try {
			...
			foreach ($rsdata as $value) {
				...
				$factura_procesada[$cli_anticipo][$ch_cliente][$ch_sucursal][$art_codigo]['cantidad']+=$value['nu_cantidad'];
