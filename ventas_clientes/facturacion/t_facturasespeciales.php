...
class FacturasTemplate extends Template {
  ...
  function titulo(){
    $titulo = '<div align="center"><h2>Facturaci&oacute;n Especial</h2></div><hr>';
    return $titulo;
  }
  ...
  function TmpReportePDF($datos)
  {
	...
	if($datos)
	{
		...
		$columnas = array('FECHA DE MOVIMIENTO' => 'FECHA DE MOVIMIENTO',
						  'SERIE DE DOCUMENTO' => 'SERIE DE DOCUMENTO',
		...
		$reporte->definirColumna("FECHA DE MOVIMIENTO", $reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("SERIE DE DOCUMENTO", $reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("TIPO DE DOCUMENTO", $reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("NUMERO DE DOCUMENTO",$reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("VALOR_VENTA", $reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("IGV", $reporte->TIPO_TEXTO, 20, "L");
		$reporte->definirColumna("TOTAL_VENTA",$reporte->TIPO_TEXTO, 15, "L");
		$reporte->definirColumna("ANTICIPO",$reporte->TIPO_TEXTO, 20, "L");
		...
		for($j=0;$j<count($datos);$j++)
		{
			if (($datos[$j-1][0] != $datos[$j][0]) && $row != $contador)
			{
			 ...
			 $datos['FECHA DE MOVIMIENTO'] = $datos[$j]['fecha'];
			 $datos['SERIE DE DOCUMENTO'] = $datos[$j]['serie'];
			 $datos['TIPO DE DOCUMENTO'] = $datos[$j]['tipo'];
			 $datos['NUMERO DE DOCUMENTO'] = $datos[$j]['numero'];
			 $datos['VALOR_VENTA'] = $datos[$j]['valor_venta'];
			 $datos['IGV'] = $datos[$j]['igv'];
			 $datos['TOTAL_VENTA'] = $datos[$j]['total_venta'];
			 $datos['ANTICIPO'] = $datos[$j]['anticipo'];
			 ...
			 {
				$total['NUMERO DE DOCUMENTO'] = "TOTAL MOV. CLIENTE : ";
...
////////////////////FUNCION FORM REPORTE///////////////
  ...
  function formReporte()
  {
    ...
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'FACTURACION.ESPECIALES'));
	...
	$form->addElement(FORM_GROUP_MAIN, new f2element_text ('busqueda[fecha_ini]','Fecha de Inicio :', $_REQUEST['busqueda']['fecha_ini']?@$_REQUEST['busqueda']['fecha_ini']:@$fecha_ini, '', 20, 18));
	$form->addElement(FORM_GROUP_MAIN, new f2element_text ('busqueda[fecha_fin]','Fecha Fin :', $_REQUEST['busqueda']['fecha_fin']?@$_REQUEST['busqueda']['fecha_fin']:@$fecha_fin, '', 20, 18));
  ...
  $Codigo = @$_REQUEST['paginacion']['codigo']?@$_REQUEST['paginacion']['codigo']:@$_REQUEST['busqueda']['codigo'];
  $F_Desde = @$_REQUEST['paginacion']['f_desde']?@$_REQUEST['paginacion']['f_desde']:@$_REQUEST['busqueda']['f_desde'];
  $F_Hasta = @$_REQUEST['paginacion']['f_hasta']?@$_REQUEST['paginacion']['f_hasta']:@$_REQUEST['busqueda']['f_hasta'];
  $VarBusqueda = '&busqueda[codigo]='.$Codigo.'&busqueda[f_desde]='.$F_Desde.'&busqueda[f_hasta]='.$F_Hasta.'';
    ...
    $listado .= '<th>'.espacios(10).'</th><th>'.espacios(4).'</th></tr><tbody class="grid_body" style="height:250px;">';
    ...
    foreach($registros as $reg){
      ...
      $listado .= '<td><A href="control.php?rqst=FACTURACION.ESPECIALES&task=ESPECIALES&montorecargo='.trim($reg[19]).'&recargo='.trim($reg[18]).
                  ''.$VarBusqueda.'&action=Modificar&registroid='.trim($reg[0]).' '.trim($reg[1]).' '.trim($reg[2]).' '.trim($reg[3]).'" target="control"><img src="/sistemaweb/icons/open.gif" alt="Editar" align="middle" border="0"/></A>&nbsp;';
      if ($_SESSION["autorizacion"]){   
	      $listado .= '<A href="javascript:confirmarLink(\'Desea eliminar el documento '.$regCod.'?\',\'control.php?rqst=FACTURACION.ESPECIALES&task=ESPECIALES'.
	                  ''.$VarBusqueda.'&action=Eliminar&registroid='.trim($reg[0]).' '.trim($reg[1]).' '.trim($reg[2]).' '.trim($reg[3]).'\', \'control\')"><img src="/sistemaweb/icons/delete.gif" alt="Borrar" align="middle" border="0"/></A>';
	      $listado .= '<A href="javascript:confirmarLink(\'Desea Anular el documento '.$regCod.'?\',\'control.php?rqst=FACTURACION.ESPECIALES&task=ESPECIALES'.
	                  ''.$VarBusqueda.'&action=Anular&registroid='.trim($reg[0]).' '.trim($reg[1]).' '.trim($reg[2]).' '.trim($reg[3]).'\', \'control\')"><img src="/sistemaweb/icons/anular.gif" alt="Anular" align="middle" border="0"/></A><td>';
      }            
      $listado .= '</tr>';
     }
  ...
  $Codigo = @$_REQUEST['busqueda']['codigo'];
  ...
  $F_Hasta = @$_REQUEST['busqueda']['f_hasta']?@$_REQUEST['busqueda']['f_hasta']:@$_REQUEST['paginacion']['f_hasta'];
   //echo "ENTRO BUSCAR\n";
    ...
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('<center>'));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('busqueda[codigo]','Codigo :', @$_REQUEST["paginacion"]["codigo"], espacios(2), 20, 18, array("onkeyup"=>"javascript:this.value=this.value.toUpperCase();")));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('busqueda[f_desde]','Desde :', @$_REQUEST["paginacion"]["f_desde"]?@$_REQUEST["paginacion"]["f_desde"]:@$f_desde, espacios(2), 10, 10));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('busqueda[f_hasta]','Hasta :', @$_REQUEST["paginacion"]["f_hasta"]?@$_REQUEST["paginacion"]["f_hasta"]:@$f_hasta, espacios(2), 10, 10));
  ...
  function formFacturas($datos, $DatosArrayArticulos)
  {
    ...
    if(empty($datos['dt_fac_fecha']) && !$datos["registroid"]){
        ...
        $fecha_tp = $fechaDiv[1]."/".$fechaDiv[2]."/".$fechaDiv[0];
    }
    ...
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('paginacion[f_desde]', @$_REQUEST["busqueda"]["f_desde"]));
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('paginacion[f_hasta]', @$_REQUEST["busqueda"]["f_hasta"]));
    ...
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>'));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('datos[dt_fac_fecha]','Fecha'.espacios(2).'</td><td>: ', @$datos["dt_fac_fecha"], '', 10, 10,array(),array($deshabilitar)));
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>'));
    ...
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('datos[ch_liquidacion]','Liquidacion: </td><td valign="top" rowspan="8">', @trim($datos["ch_liquidacion"]), '', 12, 12, array("class"=>"form_input_numeric", "onKeyPress"=>"return validar(event,2);"),array($deshabilitar)));
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>'));
    ...
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>'));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('datos[nu_tipocambio]','T. Cambio</td><td>: ', @$datos['nu_tipocambio'], '', 5, 5,array("class"=>"form_input_numeric"), array($deshabilitar)));
    $form->addElement(FORM_GROUP_MAIN, new f2element_freeTags('</td></tr><tr><td>'));
    $form->addElement(FORM_GROUP_MAIN, new f2element_text ('datos[ch_factipo_descuento1]','Dscto</td><td>: ', @trim($datos["ch_factipo_descuento1"]), '', 3, 3, array("onKeyUp"=>"getRegistroDesc(this.value);", "class"=>"form_input_numeric", "onKeyPress"=>"return validar(event,2);"),array($deshabilitar)));
    ...
    if ($_REQUEST['action']!='Modificar'){
	    ...
	    $form->addElement('articulos', new f2element_textarea ('interface[dato_articulo][desc_articulo][]','', '', '', 30, 3, array("class"=>"form_input","onKeyUp"=>"this.value=this.value.toUpperCase();","onKeyDown"=>"return maximaLongitud(this,199)")));
	    $form->addElement('articulos', new f2element_freeTags('</td><td>'));
	    ...
	    $form->addElement('articulos', new f2element_freeTags('</td><td>'));
	    $form->addElement('articulos', new f2element_text ('interface[dato_articulo][precio_articulo][]','', '', '', 8, 10, array("onKeyUp"=>"CalcularValores();", "class"=>"form_input_numeric", "onKeyPress"=>"return validar(event,3);")));
	    $form->addElement('articulos', new f2element_freeTags('</td><td>'));
	    ...
	    $form->addElement('articulos', new f2element_freeTags('</td><td>'));
	    $form->addElement('articulos', new f2element_text ('interface[dato_articulo][dscto_articulo][]','', '', '', 8, 10, array("class"=>"form_input_numeric")));
	    $form->addElement('articulos', new f2element_freeTags('</td><td>'));
	    $form->addElement('articulos', new f2element_text ('interface[dato_articulo][total_articulo][]','', '', '', 8, 10, array("class"=>"form_input_numeric",'onKeyUp'=>'CalcularValores();')));
    ...
    if ($_REQUEST['action']!='Modificar'){
    	...
    	if ($_SESSION['autorizacion']){
    		...
    		$form->addElement('buttons', new f2element_button('action','Autorizar', espacios(2),array('onClick'=>"volver_para_autorizar('".$_REQUEST['registroid']."');")));
    	} 
    }
  ...
  {
  	if ($_REQUEST['action']=='Modificar') $deshabilitar = 'class="form_input_numeric"';
  	else $deshabilitar = 'readonly class="form_input_numeric_disabled" ';
    ...
    foreach($lista as $llave => $valor)
    {
		...
		$formulario .= '<tr valign="top"><td>'."\n".
		    ...
		    '<input type="text" name="cod_articulo[]" value="'.trim($valor['cod_articulo']).'" '.$deshabilitar.' onKeyPress="return validar(event,2);" size="20">'.
		    '</td><td>'."\n".
		    '<input type="text" name="desc_articulo[]" value="'.trim($valor['desc_articulo']).'" '.$deshabilitar.' size="25">'.
		    '</td><td>'."\n".
		    '<input type="text" name="cant_articulo[]" value="'.money_format("%.2n",round($valor['cant_articulo'],2)).'" '.$deshabilitar.' onKeyPress="return validar(event,3);" size="5">'.
		    '</td><td>'."\n".
		    '<input type="text" name="precio_articulo[]" value="'.$valor['precio_articulo'].'" '.$deshabilitar.' onKeyPress="return validar(event,3);" size="8">'.
		    '</td><td>'."\n".
		    '<input type="text" name="neto_articulo[]" value="'.$valor['neto_articulo'].'" '.$deshabilitar.'  onKeyPress="return validar(event,3);" size="8">'.
		    '</td><td>'."\n".
		    '<input type="text" name="igv_articulo[]" value="'.$valor['igv_articulo'].'" '.$deshabilitar.' onKeyPress="return validar(event,3);" size="8">'.
		    '</td><td>'."\n".
		    '<input type="text" name="dscto_articulo[]" value="'.$valor['dscto_articulo'].'" '.$deshabilitar.' onKeyPress="return validar(event,3);" size="8">'.
		    ' - '.round(($valor['dscto_articulo']*100)/(money_format("%.2n",round($valor['cant_articulo'],2))*$valor['precio_articulo']),2).'% </td><td>'."\n".
		    '<input type="text" name="total_articulo[]" value="'.$valor['total_articulo'].'" '.$deshabilitar.' onKeyPress="return validar(event,3);" size="8">'.
		    '</td><td>'."\n".
  ...
  if($_SESSION["TOTAL_ARTICULOS"]>0){
    ...
    $formulario .= '<tr valign="top"><td>'."\n".
                   ...
                   '</td></tr><tr valign="top"><td>ISC: <input type="text" name="datos[nu_fac_impuesto2]" value="'.@$datos['nu_fac_impuesto2'].'" class="form_input_numeric" size="7" onKeyPress="return validar(event,3);" > 
                   TOTALES : '."\n".
                  '</td><td>&nbsp;'."\n".
                  '</td><td>'."\n".
                  '<input type="text" name="datos[nu_fac_valorbruto]" value="'.money_format("%.2n", $lista['total_neto_articulo']).'" class="form_input_numeric" onKeyPress="return validar(event,3);" size="8">'.
                  '</td><td>'."\n".
                  '<input type="text" name="datos[nu_fac_impuesto1]" value="'.money_format("%.2n", $lista['total_igv_articulo']).'" class="form_input_numeric" onKeyPress="return validar(event,3);" size="8">'.
                  '</td><td>'."\n".
                  '<input type="text" name="datos[nu_fac_descuento1]" value="'.money_format("%.2n", $lista['total_dscto_articulo']).'" class="form_input_numeric" onKeyPress="return validar(event,3);" size="8">'.
                  '</td><td>'."\n".
                  '<input type="text" name="datos[nu_fac_valortotal]" value="'.money_format("%.2n", round($lista['total_total_articulo'],2)).'" class="form_input_numeric" onKeyPress="return validar(event,3);" size="8">'.
                  '</td><td>&nbsp;&nbsp;'."\n".
                  '</td>'."\n".
                  '</tr>'."\n";
    $formulario .= '</table>'."\n";
  ...
  function setRegistros($codigo, $codtdoc)
  {
    ...
    if (count($RegistrosCB['Datos']) == 1) {
      foreach($RegistrosCB['Datos'] as $cod => $descri){
        $result = $descri." <script language=\"javascript\">top.setRegistro('".trim($cod)."', '".$RegistrosCB['Numeros'][trim($cod)]."','".$RegistrosCB['Almacen'][trim($cod)]."');</script>";
      }
    }
    if (count($RegistrosCB['Datos']) > 1){
      $att_opt = array();
