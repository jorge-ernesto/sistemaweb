...
class CReportes2 extends FPDF
{
    ...
    var $TIPO_ENTERO = 4;		/* sin decimales, coma para separar miles */

    var $separador = " ";		/* Caracter que separa las columnas */

    var $cab_default = Array();

	function getCReportes2(){
	    return new FPDF();
	}
    ...
    function nuevaFila($valores=Array(), $template="_default", $bCabecera=false, $wordText=false){
		...
		//var_dump($texto);
        //$this->Multicell(0,$this->FontSizePt,wordwrap($texto, 40, "\n"));
    ...
    function borrarCabeceras()
    {
	$this->templates = Array();
	$this->cabecera = Array();
	$this->borrarCabeceraPredeterminada();
    }
    
    function borrarCabeceraPredeterminada()
    {
	$this->cab_default = Array();
    }
		...
		foreach($this->templates[$template] as $nombre=>$config) {
		    ...
		    if (!$bCabecera) {
				switch ($config['tipo']) {
					...
					break;
				    case $this->TIPO_ENTERO:
						if ($valores[$nombre] != "") $value = number_format($valores[$nombre], 0, ".", ",");
						else $value = "";
					break;
				    default:
...
*/
	...
	/* Hora */
	$str = str_replace("%h", date("H:i:s"), $str);
	
	/* Usuario */
	$str = str_replace("%u", $usuario->obtenerUsuario(), $str);
