<?php
  /*
    Models para diversos casos en sistema acosa
...
class VariosModel extends Model {
  ...
  function almacenCBArray(){
    ...
    //print_r($cbArray);
    return $cbArray;
    //return array("001"=>"Principal", "002"=>"Sucursal");
  }
  ...
  function sucursalCBArray(){
    global $sqlca;
    $query = "SELECT ch_sucursal, 
                     ch_sucursal||' '||ch_nombre_breve_sucursal 
              FROM int_ta_sucursales 
              ORDER BY ch_sucursal";
//              WHERE ch_tipo_sucursal = 'E' 
  ...
  {
    global $sqlca;
    $query = "SELECT DISTINCT a.ch_tanque,
		    a.ch_tanque||' '||b.ch_nombrecombustible
	      FROM  comb_ta_tanques a,
		  comb_ta_combustibles b
	     WHERE a.ch_codigocombustible=b.ch_codigocombustible 
  ...
  {
    //Asignar los códigos de artículos a los tanques correspondientes
    $cbArray = array();
    $cbArray['01'] = '11620301';
    $cbArray['02'] = '11620305';
    $cbArray['03'] = '11620304';
  ...
  {
  global $sqlca;
    if($CodDes==true) $addQuery = " trim(tab_elemento)||' '||trim(tab_descripcion)"; 
    else $addQuery = " tab_descripcion"; 
    ...
    //echo "QUERY : $query \n";
    $cbArray = array();
    if(!empty($valDef)) $cbArray[''.$valDef['value'].'']=''.$valDef['descripcion'].'';
  ...
  {//@@ Inicio - Funciòn para generar el arreglo de los Días

    $cbArray= array();//@@ Iniciar el Arreglo
    $dia    = date ("d"); //@@ Dìa Actual
    ...
    $ultimo_dia=31; // @@ Último Día en el caso del mes de Febrero y Años Bisiestos
    
    //@@ Generar el arreglo del total de días del mes correcpondiente
    ...
    return $cbArray;//@@ Retornar el Arreglo Generado.
    
  }//@@ Fin - Funciòn para generar el arreglo de los Días
  
  function aniosCBArray()
  {
    $cbArray=array();
    $anio_actual   = date ("Y");
    //echo "AÑO : $anio_actual \n";
  ...
  global $sqlca;
    ...
    $registro = $sqlca->firstRow($query);
    if(!$registro){ return "ERROR: CODIGO NO GENERADO"; };
    return $registro[0];
  }
...
/*  function ObtenerIPAlmacen($CodAlmancen)
  ...
  global $sqlcabk;
    $query = "SELECT ".
                    "ip ".
             "FROM int_estaciones ".
             "WHERE lpad(id, 3, '0') = '".$CodAlmancen."'";

    $registro = $sqlcabk->firstRow($query);
    if(!$registro){ return "ERROR: NO EXISTE ALMACEN"; };
    return $registro[0];
  }
