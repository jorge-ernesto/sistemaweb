...
Class AgentePercepcionViModel extends Model{

  function eliminarRegistros()
  {
    global $sqlca;
    if($sqlca->perform('sunat_agente_percepcion_vi  ', ' ', 'delete', "ch_ruc > '0'")>=0){
  ...
  {
    global $sqlca;
    if($sqlca->perform('sunat_agente_percepcion_vi', $datos, 'insert')>=0){
    } else { return $sqlca->get_error();}
    return OK;
  }
...
//ch_ruc,ch_nombre_razon_social,dt_fecha_inicio,nu_resolucion
    $query = "SELECT  ch_ruc, ".
		     ...
		     "dt_fecha_inicio, ".
		     "nu_resolucion ".
             "FROM sunat_agente_percepcion_vi ".
	     " ".$cond." ".
  ...
  global $sqlca;
   ...
   if($ext=='txt' || $ext== 'csv' || $ext== 'zip')
   {
	...
	{//INICIO :: IF - Copiar el file al Servidor
	    ...
	    if(!empty($CampoArray))
	    {
                AgentePercepcionViModel::eliminarRegistros();
		...
		AgentePercepcionViModel::AgregarRegistros($datos[$key]);
	    }
