...
<html>
    <head>
        ...
        <script type="text/javascript">
			...
			var MIN_LENGTH 				= 1;
           	...
           	function verdetalle(id_recibo){
                window.open ("informe_registro_caja.php?id_recibo="+id_recibo+"", "mywindow","location=1,status=1,scrollbars=1, width=950,height=600")
            }
            
            function anular(id_recibo, nu_almacen, dEntry){
		...
		//ELIMINAR DETALLE DE MEDIO DE PAGO A LA HORA DE AGREGAR
			...
			if(tipo_moneda == "01")
		        ...
		        if(estado=="1"){
                  ...
                  //       } else {
						    ...
						    html_sin_cliente+="<th><input type='text' id='txtimporte_tmp'/></th>";
		...
		//FIN DE OPERACION DE CLIENTE O TICKET RECIBO
		...
		//TABLA DE AGREGAR RECIBO SIN CLIENTE
		...
		//BUSCADOR DE CLIENTE AUTOMATICO
		...
		.focus(function() {		
                ...
                $('#id_nuevo_registro').click(function(){
                    ...
                    $.ajax({
         			...
         			//CAMBIADO GEANCARLOS OBTNER T.C.
		...
		//FIN CLIENTE DOCUMENTOS POR COBRAR

		//AGREGAR DOCUMENTOS POR COBRAR PARA PAGAR
		...
		$('#btnselec_elem').off('click');
			...
			$(document).on('click','#btnselec_elem',function(){
				...
				if(valor_dni == "" || valor_dni.length == 0){
					alert('Error al Buscar Documentos del Cliente');
				}else{
		...
		$(document).on('click','#guardartmpcliente',function(){
			...
			if(estado_validacion=="1"){
                        	...
                        	if(txtnum_referencia.trim().length==0){
                            		alert('Ingresar Numero de Referencia');
                            		estado_ejecucuion=false; 
				}
				...
				if(cuentas_cmb_mostrar=="-" ){
                            		alert('Seleccionar Numero de cuenta');
                            		estado_ejecucuion=false;
				}
		...
		$('#finalizarproceso').off('click'); //Finalizar Proceso para realizar pago de cuenta por cobrar
			...
			if($.fn.validarGeneral()==true){
		                ...
		                //GUARDAMOS DATA DE LAS CAJAS DE TEXTO;
