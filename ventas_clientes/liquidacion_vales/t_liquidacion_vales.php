...
class LiquidacionValesTemplate extends Template {
	...
	<div class='contenedorprincipal'>
		<div ><h3 style="color: #336699; text-align: center;">PROCESO DE LIQUIDACION DE VALES</h3></div>
        ...
        <div  class="separacion">
            ...
            <div  class="separacion">
                <div class='fila'>
                    ...
                    <div style="float: left;"><input type='text' id='fecha_liqui' class='fecha_formato'/></div>
                </div>

            <?php
            if ($selectexonerada==true){
                ?>
             <div  class="separacion">
                <div class='fila'>
                    <div class='etiquetavales' style="float: left;">Exonerado </div>
                    <div style="float: center;"><input type='checkbox' id='sin_igv' class='fecha_formato' onclick="clicktrans_gratis('trans_gratis')" /></div>
                </div>
            </div>
            <?php
                }
        ...
        <!--               <tr>
             ...
             <div  class="separacion">
                <div class='fila'>
                    <div class='etiquetavales' style="float: left;">Transferencia Gratuita </div>
                    <div style="float: center;"><input type='checkbox' id='trans_gratis'  class='fecha_formato' onclick="clicksin_igv('sin_igv')" /></div>
                    <div class='etiquetavales' style="float: left;">Considerar Negativos </div>
                    <div style="float: center;"><input type='checkbox' id='cons_nega' class='fecha_formato'/></div>
                </div>
            </div>

		<div class="separacion">
		        <div class='etiquetavales' style="float: left;margin-bottom: 5px;text-align: center;width: auto;"><button id="btnseleccionar"><img align="right" src="/sistemaweb/images/search.png"/>Buscar Clientes</button></div>
		        <div class='etiquetavales' style="float: left;margin-bottom: 2px;margin-right: 2px;text-align: center;width: auto;"><button id="btnbuscarliquidacion"><img align="right" src="/sistemaweb/images/search.png">Buscar Liquidaciones</button></div>
		        <div class='etiquetavales' style="float: left;margin-bottom: 5px;text-align: center;width: auto;"><button id="btnliquidar"><img align="right" src="/sistemaweb/images/MasterDetail.gif">Liquidar vales</button></div>
		</div>
    ...
    function CrearTablaVervales($registros, $datos_cliente, $fecha_inicio, $fecha_final, $vales_sele) { ?>
        ...
        <!--<div style="float:left; width: 400px;margin-bottom: 13px;">
            <div style="font-size: 13px;font-weight: bold;float: left;width: 120px;margin-bottom: 5px;">Codigo Cliente  :</div>
            <div style="font-size: 12px;text-transform: capitalize;float: left;width: 250px;;margin-bottom: 5px;"><?php echo $datos_cliente['cli_codigo']; ?>
                <input type="hidden" id="idcodigoCliente" value="<?php echo $datos_cliente['cli_codigo']; ?>"/>
                <input type="hidden" id="accionInterno" value="unicoCliente"/>
                <input type="hidden"  id="fecha_inicio_tmp" value="<?php echo $fecha_inicio; ?>"/>
                <input type="hidden"  id="fecha_final_tmp" value="<?php echo $fecha_final; ?>"/>
            ...
            <div style="font-size: 13px;font-weight: bold;float: left;width: 120px;margin-bottom: 5px;">Ruc Cliente  :</div>
            <div style="font-size: 12px;text-transform: capitalize;float: left;width: 250px;;margin-bottom: 5px;"><?php echo $datos_cliente['cli_ruc']; ?> </div>

            <div style="font-size: 13px;font-weight: bold;float: left;width: 120px;margin-bottom: 5px;">Razon Social :</div>
            <div style="font-size: 12px;text-transform: capitalize;float: left;width: 250px;;margin-bottom: 5px;"><?php echo $datos_cliente['cli_razsocial'] ?> </div>

            <div style="font-size: 13px;font-weight: bold;float: left;width: 120px;margin-bottom: 5px;">Anticipo :</div>
            <div style="font-size: 12px;text-transform: capitalize;float: left;width: 250px;;margin-bottom: 5px;"><?php echo $datos_cliente['cli_anticipo'] ?> </div>

            <div style="font-size: 13px;font-weight: bold;float: left;width: 120px;margin-bottom: 5px;">Ordenar Chofer:</div>
            <div style="font-size: 12px;text-transform: capitalize;float: left;width: 250px;;margin-bottom: 5px;">
                <input type="checkbox" id="nochofer" name="nochofer" class="chofer" onclick="OrdenarChofer(this)" value="S">
            </div>
        </div>-->
            ...
            <table align="center" cellspacing="0" cellpadding="0" border="0">	
                ...
                <tbody>
                    ...
                    foreach ($registros as $llave => $value) {
                        ...
                        //echo "<td>" . $value['fecha_replicacion'] . "</td>";
                        ...
                        echo "<td class='td_tabla_selecinar' style='text-align:center'>
                        		<input type = 'checkbox' $ckecked id='".$value['ch_documento']."' class = 'idselecAll ".$nochofer."' cantidad = '" . $nu_cantidad . "' importe = '" . $nu_importe . "' onclick='marcarHermanos(this, \"" .$nochofer."\")' value='" . trim($value['ch_documento']) . "'/></td>";
                        //echo "<td class='td_tabla_selecinar' style='text-align:center'><input type='checkbox' $ckecked id='".$nochofer."' cantidad='" . $nu_cantidad . "'  importe='" . $nu_importe . "' onclick='marcarHermanos(this, \"" .$nochofer."\")' class='idselecAll' value='" . trim($value['ch_documento']) . "'/></td>";
                        //echo "<td class='td_tabla_selecinar' style='background-color:#C4C4C4'></td>";
    ...
    function CrearTabladatosLiquidacionProducto_Busqueda($registros,$taxOptional) {
        ...
        <div style="float: left;width: auto;border: 1px;">
            <table cellspacing="1" cellpadding="0" border="0">	
                ...
                <?php if($taxOptional) { ?><th class="th_cabe" align="center">IGV</th><?php } ?>
                ...
                <tbody>
                    ...
                    foreach ($registros as $llave => $value) {
                        ...
                        echo "<td align='center'>" . $value['desc_tipodocumento'] . " - " . $value['ch_fac_seriedocumento'] . " - " . $numero_docdumento . "</td>";
                        ...
                        //echo '<a href="javascript:void(0)" onClick="GenerarFactura('.trim($tipo_doc).',\''.trim($value['ch_fac_seriedocumento']).'\',\'' . trim($value['ch_fac_numerodocumento']) . '\', \''.trim($value['ch_cliente']).'\','.$_id.')"><img src="/sistemaweb/images/icono_imprimir.gif"/></a>';
                            //echo '<a target="_blank" href="generar_lv_print.php?liquidacion='.trim($value['ch_liquidacion']).'&cliente='.trim($value['ch_cliente']).'&serie='.trim($value['ch_fac_seriedocumento']).'&documento='.trim($numero_docdumento).'&tipoDocumento='.trim($tipo_doc).'"><img src="/sistemaweb/images/icono_imprimir.gif"/></a>';//kwn
                        if($value['operacion']!="POR-COBRAR"){
                            if(preg_match("/^[a-zA-Z][0-9]{3}+$/", trim($value['ch_fac_seriedocumento'])) == 1) {
                                echo '<a style="cursor: pointer;" data-documento="'.trim($numero_docdumento).'" onclick="generarDocumentoLV('.$_id.',\''.trim($tipo_doc).'\',\''.trim($value['ch_fac_seriedocumento']).'\',\''.trim($value['ch_fac_numerodocumento']).'\','.($taxOptional ? 1 : 0).')"><img src="/sistemaweb/images/icono_imprimir.gif"/></a>';//kwn
                            }else{
