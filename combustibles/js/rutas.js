function nuevoAjax(){ //esto siempre va define tipo de navegador, si soporta y se puede agregar mas catch segun uno lo necesite....
...
function cargarContenido(codpedido,ruc,fhd,placa,idplaca,galo,i){
       ...
       ajax = nuevoAjax();
alert(idplaca);
       ajax.open("POST", "control.php?rqst=MOVIMIENTOS.PROGRAMRUTA&action=Actualizar&codpedido="+codpedido+"&ruc="+ruc+"&fhd="+fhd+"&placa="+placa+"&idplaca="+idplaca+"&galo="+galo,true); 
