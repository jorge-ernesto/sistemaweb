...
class AgenteBuencontrTemplate extends Template {
 
  function titulo(){
    $titulo = '<div align="center"><h2>Buen Contribuyente</h2></div><hr>';
    return $titulo;
  }
  ...
  function listado($registros){
    ...
    $titulo_grid = "BUEN CONTRIBUYENTE";
  ...
  function formBuscar($paginacion){
   //echo "ENTRO BUSCAR\n";
    ...
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MAESTROS.SUNAT_BUENCONTR'));
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'SUNAT_BUENCONTR'));
  ...
  function formAgenteBuencontr($datos)
  {
    
    $form = new form2('BUEN CONTRIBUYENTE', 'form_agen_ret', FORM_METHOD_POST, 'control.php', 'multipart/form-data', 'control');
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('rqst', 'MAESTROS.SUNAT_BUENCONTR'));
    $form->addElement(FORM_GROUP_HIDDEN, new f2element_hidden('task', 'SUNAT_BUENCONTR'));
