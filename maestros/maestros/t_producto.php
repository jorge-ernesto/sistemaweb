<?php
class ProductoTemplate extends Template {
	...
	function formSearch($fecha,$fecha2,$paginacion){
		...
		$form->addElement(FORM_GROUP_HIDDEN, new form_element_hidden("rqst", "MAESTROS.PRODUCTO"));
	...
	function resultadosBusqueda($resultados) {
		...
		for ($i = 0; $i < count($resultados); $i++) {
			...
			$result .= '<td class="'.$color.'" align="center"><A href="control.php?rqst=MAESTROS.PRODUCTO&action=Modificar&codigo_combu='.htmlentities($a['ch_codigocombustible']).'&nombre_combu='.htmlentities($a['ch_nombrecombustible']).'" target="control"><img src="/sistemaweb/icons/open.gif" align="middle" border="0"/></A>&nbsp;</td>';
			$result .= '<td><A href="javascript:confirmarLink(\'Desea eliminar la matricula?\',\'control.php?rqst=MAESTROS.PRODUCTO&task=PRODUCTO&action=Eliminar&registroid='.$regCod.'&fecha='.$regFecha.'&turno='.$regTurno.'&codIsla='.$regCodIsla.'&codTrab='.$regCodTrab.'&tipo='.$regTipo.'\', \'control\')"><img src="/sistemaweb/icons/delete.gif" alt="Borrar" align="middle" border="0"/></A></td>';
			$result .= '<td class="'.$color.'" align="center"><A href="control.php?rqst=MAESTROS.PRODUCTO&action=Eliminar&codigo_combu='.htmlentities($a['ch_codigocombustible']).'" target="control"><img src="/sistemaweb/icons/delete.gif" align="middle" border="0"/></A>&nbsp;</td>';			
			$result .= '</tr>';
